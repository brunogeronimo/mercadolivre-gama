...
if(isset($_GET["id"]) && Server::InitDataProvider())
{
    ...
    if(Operator::IPValidate())
    {
        if(empty(Server::$Configuration->File["gl_avhe"]))
            exit("HTML content is currently not being saved for security reasons. Please check your LiveZilla configuration:<br><br>LiveZilla Server Admin -> Server Configuration -> Server -> Security");
        $c=null;
