...
function buildResources($xml = "", $last = 0)
{
    if(isset($_POST[POST_INTERN_XMLCLIP_RESOURCES_END_TIME]))
    {
        ...
        $count = 0;
        if($result = DBManager::Execute(true, "SELECT * FROM `" . DB_PREFIX . DATABASE_RESOURCES . "` WHERE `edited` > " . DBManager::RealEscape($_POST[POST_INTERN_XMLCLIP_RESOURCES_END_TIME]) . " AND `edited`<=" . DBManager::RealEscape(time()) . " AND `parentid` NOT IN (100,101,102) ORDER BY `edited` ASC"))
        {
...
function demandObjectData()
{
    ...
    {
        $chatListingMode = isset($_POST[POST_INTERN_DUT_VISITORS_FULL_DATA_CHATS_ONLY]);
...
function demandEmails($xml="",$count=0,$lmc=0,$c_name="",$c_text="")
{
    ...
    if($count>0)
    {
        ...
        if($result && $row = DBManager::FetchArray($result))
        {
            ...
            $c_name = Str::Cut((!empty($sn) ? $sn : $row["sender_email"]),90);
            $c_text = Str::Cut(DBManager::DecodeField($row["body"]),90);
        }
    }
...
{
    if(!CacheManager::IsDataUpdate(POST_INTERN_DUT_CHAT_ARCH,DATA_UPDATE_KEY_CHAT_ARCH,300))
        return;
    ...
    else
    {
        ...
        if(!empty($_POST["p_dc_q"]))
        {
            ...
            $q_searchw = ChatArchive::GetSearchQuery($q);
...
function demandTickets($xml="",$q_content_filter="",$q_count_filter="",$q_searchw="",$q_searchf="",$c_total=0,$c_totalall=0,$c_totalread=0,$c_totalquery=0,$c_totalmy=0,$c_totalmygroups=0,$c_totalst0=0,$c_totalst1=0,$c_totalst2=0,$c_totalst3=0,$c_totaltsd="",$c_lmc=0,$c_name="",$c_text="",$wlxml = "",$loads=0)
{
    ...
    if($permission != PERMISSION_NONE)
    {
        ...
        if(!empty($_POST["p_dt_q"]))
        {
            ...
            $q_searchw = Ticket::GetSearchQuery($q,isset($_POST["p_dt_q_u"]));
        ...
        if(!empty(Server::$Configuration->Database["gl_tsd"]))
            foreach(Server::$Configuration->Database["gl_tsd"] as $tsd)
                $q_count["totaltsd"] .= ",(SELECT COUNT(*) FROM `".DB_PREFIX.DATABASE_TICKETS."` LEFT JOIN `".DB_PREFIX.DATABASE_TICKET_EDITORS."` AS `te` ON `".DB_PREFIX.DATABASE_TICKETS."`.`id`=`te`.`ticket_id` WHERE `status`='".DBManager::RealEscape($tsd->ParentId)."' AND `sub_status`='".DBManager::RealEscape($tsd->Id)."' ".$q_grperm.$q_count_filter.") AS `ttsd".$tsd->GetShortId()."`";

        $result = DBManager::Execute(true, $d=($q_count["total"] . "," . $q_count["totalperm"] . "," . $q_count["totalquery"] . $q_count["totalread"] . $q_count["totalall"] . $q_count["totalmy"] . $q_count["totalmygroups"] . $q_count["totalst0"] . $q_count["totalst1"] . $q_count["totalst2"] . $q_count["totalst3"] . $q_count["totaltsd"] . " FROM `" . DB_PREFIX . DATABASE_TICKETS . "` WHERE `deleted`=0;"));
        if($row = DBManager::FetchArray($result))
        {
            ...
            $result = DBManager::Execute(true, "SELECT `t2`.`time` AS `lmc`,`fullname`,`text` FROM `" . DB_PREFIX . DATABASE_TICKETS . "` AS `t1` INNER JOIN `" . DB_PREFIX . DATABASE_TICKET_MESSAGES . "` AS `t2` ON `t1`.`id`=`t2`.`ticket_id` WHERE " . $q_grperm . "(`t2`.`type`=0 OR `t2`.`type`=3) ORDER BY `t2`.`time` DESC LIMIT 1;");
...
{
    if(!isset($_POST[POST_INTERN_XMLCLIP_CHATS_END_TIME]))
        $_POST[POST_INTERN_XMLCLIP_CHATS_END_TIME] = time()-(3*3600);

    $xml = "";
    $dut = $_POST[POST_INTERN_XMLCLIP_CHATS_END_TIME];
    ...
    $result = DBManager::Execute(true, "SELECT * FROM `".DB_PREFIX.DATABASE_VISITOR_CHATS."` WHERE (`last_active` >= " . intval($dut) . " OR `exit` >= " . intval($dut) . $modChats . ") AND `chat_id` > 0 ORDER BY `chat_id` DESC;");
    while($row = DBManager::FetchArray($result))
    {
        ...
        $dut = max($vc->LastActive,$dut);
        $dut = max($vc->Exit,$dut);

		if(strpos($modChats,$vc->ChatId)!==false)
		{
            $cp = Chat::GetPosts("",$vc->ChatId,true,false);
    	    $xml .= "<pc cid=\"".base64_encode($vc->ChatId)."\">".To::XMLTags($cp)."</pc>";
		}
    }
    Server::$Response->Chats .= "<ext_cl dut=\"".base64_encode($dut)."\">" . $xml . "</ext_cl>";
}
