...
function getFileIssues(&$_configFolderWriteable,$_updateRequired)
{
    global $LZLANG;
	$message = "";
...
function getMySQLIssues(&$_updateRequired,&$_databaseVersion)
{
    $error="";
...
function getDisabledFunctions()
{
    ...
    */
    $message = "";
    /*
    if(count(Server::$Operators) > 0 && ($miv = ((count(Server::$Groups)+count(Server::$Operators))*100)) > $currentMIV)
        $message .= "<span>PHP configuration \"max_input_vars\" (see php.ini) must be increased to ".$miv." (or greater).Your current configuration is ".$currentMIVText.".</span><br><br>";
    */
