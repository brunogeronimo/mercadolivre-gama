...
class Logging
{
    ...
    {
        $result=DBManager::Execute(true, "SELECT `ip` FROM `" . DB_PREFIX . DATABASE_ADMINISTRATION_LOG . "` WHERE `type`='Communication::SendMail' AND `ip`='" . DBManager::RealEscape(Communication::GetIP(true)) . "' AND `time`>" . intval(time() - 600));
...
class Configuration
{
    ...
    function LoadFromFile($_default=true)
    {
        ...
        if(!(isset($this->File["gl_host"]) && !empty($this->File["gl_host"])))
    ...
    function LoadFromDatabase($_extended,$_prefix)
    {
        ...
        if(!$_extended)
        {
            ...
            if(!empty($this->File["gl_stmo"]) && CALLER_TYPE != CALLER_TYPE_INTERNAL)
            {
...
class Server
{
    ...
    static function RunCronJobs($_fromApi,$_forceMaintain=false,$_forceEmailout=false,$_forceEmailin=false,$_forceSMin=false)
    {
        ...
        if($cj_email_out < (time() - (180)) && !$_fromApi)
        {
            $action = true;
        ...
        if(count(Server::$Configuration->Database["gl_email"])>0)
            if($cj_email_in < (time() - (120-(count(Server::$Configuration->Database["gl_email"])*20))) || $_forceEmailin)
            {
                ...
                Communication::DownloadEmails($_fromApi);
                if(!$_fromApi)
                    return true;
            }

        if(count(Server::$Configuration->Database["gl_sm"])>0)
            if($cj_sm_in < (time() - (120-(count(Server::$Configuration->Database["gl_sm"])*20))) || $_forceSMin)
            {
                $action = true;
                Server::SetCronjobTime("gl_cj_sm_in");
                Communication::DownloadSocialMedia($_fromApi);
                if(!$_fromApi)
                    return true;
            }
    ...
    static function CloseVisitorSessions()
    {
        ...
        if($resultv)
        {
            ...
            {
                DBManager::Execute(true, "UPDATE `" . DB_PREFIX . DATABASE_VISITORS . "` SET `closed`=".time()." WHERE `id` = '" . DBManager::RealEscape($rowv["id"]) . "';");
            }
        }
    }
    ...
    static function SetCronjobTime()
    {
        ...
        DBManager::Execute(true, $d = "REPLACE INTO `" . DB_PREFIX . DATABASE_CONFIG . "` (`key`,`value`) VALUES ('" . DBManager::RealEscape($key) . "','" . intval($time) . "');");
    ...
    static function IsManagement()
    {
        return !empty($_POST[POST_INTERN_GET_MANAGEMENT]);
    }
...
class Communication
{
    ...
    static function DownloadEmails($_fromAPI=false,$exists=false,$reload=false)
    {
        if(is_array(Server::$Groups))
            foreach(Server::$Groups as $group)
            {
                            ...
                            if(!empty($newmails) && is_array($newmails))
                                foreach($newmails as $temail)
                                {
                                    ...
                                    foreach($temail->Attachments as $attid => $attdata)
                                    {
                                        ...
                                        KnowledgeBase::Process("SYSTEM", $attid, $attdata[0], 3, $attdata[1], 0, 100);
    ...
    static function ReadParameter($_key,$_fallBack=null)
    {
        ...
        $types["int"] = array("fbt","ovlts","h","po","cid","tc","echsp","echp","ovlsx","ovlsy","ovlsb","deactr","fbshb","fbshx","fbshy","fbml","fbmt","fbmr","fbmb","fbw","fbh","ovlp","ovlw","ovlh","ovlbr","ovlml","ecw","ech","cid","ecslw","ecmb","ecfo");
    ...
    static function GetTargetParameterForm($_getParams="",$_allowed)
    {
        foreach($_GET as $key => $value)
            ...
            {
                $value = Encoding::Base64UrlEncode(Encoding::Base64UrlDecode($value));
...
class Post extends BaseObject
{
	...
	function GetXml()
	{
		...
		return "<val id=\"".base64_encode($this->Id)."\" rp=\"".base64_encode(($this->Repost) ? 1 : 0)."\" sen=\"".base64_encode($this->Sender)."\" m=\"".base64_encode($this->Micro)."\" rec=\"".base64_encode($this->ReceiverGroup)."\" reco=\"".base64_encode($this->ReceiverOriginal)."\" date=\"".base64_encode($this->Created)."\"".$translation.">".base64_encode($this->Text)."</val>\r\n";
	}
    ...
    function SaveHistory($type = 0,$iid="",$gid="")
    {
        ...
        if($type!=1)
        {
            ...
            if(($result = DBManager::Execute(true, "SELECT * FROM `" . DB_PREFIX . DATABASE_CHAT_ARCHIVE . "` WHERE `chat_id`='" . DBManager::RealEscape($id) . "';")) && $row = DBManager::FetchArray($result))
                DBManager::Execute(true, "REPLACE INTO `" . DB_PREFIX . DATABASE_CHAT_ARCHIVE . "` (`time`,`endtime`,`closed`,`chat_id`,`internal_id`,`group_id`,`html`,`plaintext`,`transcript_text`,`transcript_html`,`question`,`customs`,`subject`,`chat_type`) VALUES (" . $row["time"] . "," . time() . "," . $row["closed"] . ",'" . DBManager::RealEscape($id) . "','" . DBManager::RealEscape($iid) . "','" . DBManager::RealEscape($gid) . "','" . DBManager::RealEscape($row["html"] . $cf->GetHTMLPost($this->Text, "", time(), $this->SenderName, $this->Sender)) . "','" . DBManager::RealEscape($row["plaintext"] . "\n" . $cf->GetPlainPost($this->Text, "", time(), $this->SenderName)) . "','','','','',''," . $type . ");");
            else
                DBManager::Execute(true, "INSERT INTO `" . DB_PREFIX . DATABASE_CHAT_ARCHIVE . "` (`time`,`endtime`,`chat_id`,`internal_id`,`group_id`,`html`,`plaintext`,`transcript_text`,`transcript_html`,`question`,`customs`,`subject`,`chat_type`) VALUES (" . time() . "," . time() . ",'" . DBManager::RealEscape($id) . "','" . DBManager::RealEscape($iid) . "','" . DBManager::RealEscape($gid) . "','" . DBManager::RealEscape($cf->GetHTMLPost($this->Text, "", time(), $this->SenderName, $this->Sender)) . "','" . DBManager::RealEscape($cf->GetPlainPost($this->Text, "", time(), $this->SenderName)) . "','','','','',''," . $type . ");");
        }
    }
...
class Chat extends BaseObject
{
    ...
    function GetHTMLPost($_post,$_translation,$_time,$_senderName,$_senderId)
    {
        ...
        $avparams = ($exPost) ? "name=" . Encoding::Base64UrlEncode($_senderName) : "intid=".Encoding::Base64UrlEncode($_senderId);
        $html = str_replace("<!--avatar-->","<div style=\"background-image: url('./../picture.php?".$avparams."');\"></div>",$html);
        return $html;
    }
    ...
    static function GetPosts($_receiver, $_chatId, $_object=false,$_unreadOnly=true)
    {
        $posts = array();
        $_chatId = (!empty($_chatId)) ? " AND `chat_id`='".DBManager::RealEscape($_chatId)."'" : "";
        $_receiver = (!empty($_receiver)) ? " AND `receiver`='".DBManager::RealEscape($_receiver)."'" : "";
        $_unreadOnly = ($_unreadOnly) ? "`received`=0" : "`received`<>-1";
        if($result = DBManager::Execute(true, "SELECT * FROM `" . DB_PREFIX . DATABASE_POSTS . "` WHERE " . $_unreadOnly . $_chatId . $_receiver . " ORDER BY `time` ASC, `micro` ASC;"))
            while($row = DBManager::FetchArray($result))
                $posts[] = ($_object) ? new Post($row) : $row;
        return $posts;
    }
    ...
    static function SaveToArchive($_chatId,$_externalFullname,$_externalId,$_internalId,$_groupId,$_email,$_company,$_phone,$_host,$_ip,$_question,$_transcriptSent=false,$_waitingtime,$_duration,$_startResult,$_endResult)
    {
        ...
        DBManager::Execute(true, "UPDATE `" . DB_PREFIX . DATABASE_CHAT_ARCHIVE . "` SET `external_id`='" . DBManager::RealEscape($_externalId) . "',`closed`='" . DBManager::RealEscape(time()) . "'" . $name . ",`internal_id`='" . DBManager::RealEscape($_internalId) . "',`group_id`='" . DBManager::RealEscape($_groupId) . "',`html`='" . DBManager::RealEscape($filter->HTML) . "',`plaintext`='" . DBManager::RealEscape($filter->PlainText) . "',`transcript_text`='" . DBManager::RealEscape($tsData[0]) . "',`transcript_html`='" . DBManager::RealEscape($tsData[1]) . "',`fullname`='" . DBManager::RealEscape($_externalFullname) . "',`email`='" . DBManager::RealEscape($_email) . "',`company`='" . DBManager::RealEscape($_company) . "',`phone`='" . DBManager::RealEscape($_phone) . "',`host`='" . DBManager::RealEscape($_host) . "',`ip`='" . DBManager::RealEscape($_ip) . "',`gzip`=0,`wait`='" . DBManager::RealEscape($_waitingtime) . "',`duration`='" . DBManager::RealEscape($_duration) . "',`accepted`='" . DBManager::RealEscape($_startResult) . "',`ended`='" . DBManager::RealEscape($_endResult) . "',`transcript_sent`='" . DBManager::RealEscape(((((empty(Server::$Configuration->File["gl_soct"]) && empty(Server::$Configuration->File["gl_scct"]) && empty(Server::$Configuration->File["gl_scto"]) && empty(Server::$Configuration->File["gl_sctg"])) || empty($tsData) || $filter->ElementCount < 3 || $_transcriptSent)) ? "1" : "0")) . "',`question`='" . DBManager::RealEscape($_question) . "' WHERE `chat_id`='" . DBManager::RealEscape($_chatId) . "' AND `closed`=0 LIMIT 1;");
    ...
    static function CloseChats()
    {
        ...
        while($row = DBManager::FetchArray($result))
        {
            ...
            if($results && $rows = DBManager::FetchArray($results))
            {
                ...
                Chat::SaveToArchive($row["chat_id"],$visitor->VisitorData->Fullname,$rows["visitor_id"],$lastOp,$rows["request_group"],$visitor->VisitorData->Email,$visitor->VisitorData->Company,$visitor->VisitorData->Phone,$visitor->Host,$visitor->IP,$chat->Subject,(empty(Server::$Configuration->File["gl_sctb"]) && $botchat),$waitingtime,$duration,$startResult,$endResult);
            }
        }
    }
}
...
class Filter extends BaseObject
{
    ...
    {
        Server::InitDataBlock(array("FILTERS"));
        foreach(Server::$Filters->Filters as $currentFilter)
...
class FilterList
{
	...
	function MatchUser($_ip,$_languages,$_userid,$_country="")
	{
		...
		foreach($this->Filters as $filter)
		{
			...
			$compare["match_ip"] = Is::WildcardMatch($filter->IP,$_ip);
...
class TicketEditor extends BaseObject
{
	...
	{
		return "<cl id=\"".base64_encode($this->Id)."\" w=\"".base64_encode($_waitBegin)."\" u=\"".base64_encode($_lastUpdate)."\" st=\"".base64_encode($this->Status)."\" ss=\"".base64_encode($this->SubStatus)."\" ed=\"".base64_encode($this->Editor)."\" g=\"".base64_encode($this->GroupId)."\" ti=\"".base64_encode($this->Edited)."\"/>\r\n";
	}
	
	function Save()
	{
		DBManager::Execute(false, "UPDATE `" . DB_PREFIX . DATABASE_TICKET_EDITORS . "` SET `editor_id`='" . DBManager::RealEscape($this->Editor) . "',`group_id`='" . DBManager::RealEscape($this->GroupId) . "',`status`='" . DBManager::RealEscape($this->Status) . "',`sub_status`='" . DBManager::RealEscape($this->SubStatus) . "',`time`='" . DBManager::RealEscape(time()) . "' WHERE `ticket_id`='" . DBManager::RealEscape($this->Id) . "';");
		if(DBManager::GetAffectedRowCount() <= 0)
			DBManager::Execute(false, "INSERT IGNORE INTO `" . DB_PREFIX . DATABASE_TICKET_EDITORS . "` (`ticket_id` ,`editor_id` ,`group_id` ,`status`, `sub_status`, `time`) VALUES ('" . DBManager::RealEscape($this->Id) . "', '" . DBManager::RealEscape($this->Editor) . "','" . DBManager::RealEscape($this->GroupId) . "', '" . DBManager::RealEscape($this->Status) . "','" . DBManager::RealEscape($this->SubStatus) . "','" . DBManager::RealEscape(time()) . "');");
	}
...
class TicketMessage extends Action
{
    ...
    function AppendPostFile($_postKey,$_userId)
    {
        if(!empty($_FILES[$_postKey]) && true)
        {
            ...
            {
                KnowledgeBase::Process("SYSTEM", $fileId, $fileurid, 3, $filename, 0, 100);
    ...
    function TextReplace($_text,$_group)
    {
        ...
        $_text = str_replace("%domain%",Server::$Configuration->File["gl_host"],$_text);
        foreach(Server::$Inputs as $index => $input)
...
class TicketEmail
{
    ...
    function Save()
    {
        ...
        {
            $time=SystemTime::GetUniqueMessageTime(DATABASE_TICKET_EMAILS,"edited");
            DBManager::Execute(true, "REPLACE INTO `" . DB_PREFIX . DATABASE_TICKET_EMAILS . "` (`email_id`, `mailbox_id`, `sender_email`, `sender_name`,`sender_replyto`,`receiver_email`, `created`, `edited`, `deleted`, `subject`, `body`, `body_html`, `group_id`) VALUES ('" . DBManager::RealEscape($this->Id) . "', '" . DBManager::RealEscape($this->MailboxId) . "', '" . DBManager::RealEscape($this->Email) . "', '" . DBManager::RealEscape(DBManager::EncodeField($this->Name)) . "', '" . DBManager::RealEscape($this->ReplyTo) . "','" . DBManager::RealEscape($this->ReceiverEmail) . "', '" . DBManager::RealEscape($this->Created) . "', '" . DBManager::RealEscape($time) . "', '" . DBManager::RealEscape($this->Deleted ? 1 : 0) . "', '" . DBManager::RealEscape(DBManager::EncodeField($this->Subject)) . "', '" . DBManager::RealEscape(DBManager::EncodeField($this->Body)) . "','" . DBManager::RealEscape(DBManager::EncodeField($this->BodyHTML)) . "', '" . DBManager::RealEscape($this->GroupId) . "');", $errorCode);
        }
...
class Ticket extends Action
{
	...
	function GetXML($_full,$_demand=false)
	{
		if($_full)
		{
			...
			$xml = "<val id=\"".base64_encode($this->Id)."\" u=\"".base64_encode($this->LastUpdated)."\" w=\"".base64_encode($wb)."\" del=\"".base64_encode($this->Deleted ? "1" : "0")."\" wl=\"".base64_encode($this->WatchList ? "1" : "0")."\" gr=\"".base64_encode($this->Group)."\" l=\"".base64_encode($this->Language)."\" h=\"".base64_encode($this->GetHash())."\" p=\"".base64_encode($this->Priority)."\" t=\"".base64_encode($this->Channel)."\" s=\"".base64_encode($this->SubChannel)."\">\r\n";
    ...
    function SendOperatorReply($_messageId, $_email, $_qMessageId="")
    {
        $tsData = array("","");
        ...
        for($i=0;$i<count($tsData);$i++)
        {
            ...
            $tText = (($i!=1) ? ($reply->Text) : htmlentities($reply->Text,ENT_QUOTES,"UTF-8"));
            ...
            $tsData[$i] = str_replace("%domain%",Server::$Configuration->File["gl_host"],$tsData[$i]);
	...
	function Save($_hash="",$_saveMessages=true)
	{
		...
		if(DBManager::Execute(true, "INSERT IGNORE INTO `" . DB_PREFIX . DATABASE_TICKETS . "` (`id`,`user_id`,`target_group_id`,`hash`,`channel_type`,`sub_channel`,`iso_language`,`channel_id`,`channel_conversation_id`,`priority`) VALUES ('" . DBManager::RealEscape($this->Id) . "', '" . DBManager::RealEscape($this->SenderUserId) . "', '" . DBManager::RealEscape($this->Group) . "', '" . DBManager::RealEscape($_hash) . "', '" . DBManager::RealEscape($this->Channel) . "', '" . DBManager::RealEscape($this->SubChannel) . "', '" . DBManager::RealEscape($this->Language) . "', '" . DBManager::RealEscape($this->ChannelId) . "', '" . DBManager::RealEscape($this->ChannelConversationId) . "', " . intval($this->Priority) . ");"))
        {
    ...
    static function GetSearchQuery($_sFor)
    {
        $query = "";
        $ss = "1111111111111";
        ...
        $ss = str_split($ss);
            ...
            if($ss[12]=="1")$query .= " OR LOWER(`do`.`fullname`) LIKE '%".$_sFor."%'";
    ...
    static function GetMatchCount($_sFor)
    {
        $result = DBManager::Execute(true, "SELECT COUNT(*) as `mcount` FROM `" . DB_PREFIX . DATABASE_TICKETS . "` WHERE `user_id` LIKE '".$_sFor."'");
        if($result && $row = DBManager::FetchArray($result))
            return $row["mcount"];
        return 0;
    }
        ...
        foreach($this->Senders as $sender)
            if(empty($sender->UserSystemId) && !empty($sender->GroupId))
            {
                ...
                foreach(Server::$Operators as $sysid => $operator)
                    if($operator->IsInGroup($sender->GroupId) && !$operator->IsBot && $operator->IsExternal(Server::$Groups,null,array($sender->GroupId)))
...
class Event extends BaseObject
{
	...
	function MatchVisitorConditions($_pageCount,$_timeOnSite,$_invAccepted,$_invDeclined,$_inChat,$_searchPhrase="",$_isMobile=false,$_country="")
	{
		$match = true;
		if($_timeOnSite<0)
			$_timeOnSite = 0;
        ...
        $_result["iic"] = (!empty($this->IsInChat) && !$_inChat);
		...
		if($this->PagesVisited > 0 && $_pageCount < $this->PagesVisited)
			...
			$match = false;
        else if(!empty($this->IsInChat) && !$_inChat)
            $match = false;
	...
	function GetSQL()
	{
		return "INSERT INTO `".DB_PREFIX.DATABASE_EVENTS."` (`id`, `name`, `created`, `creator`, `edited`, `editor`, `pages_visited`, `time_on_site`, `max_trigger_amount`, `trigger_again_after`, `trigger_again_page`, `not_declined`, `not_accepted`, `not_in_chat`, `is_in_chat`, `priority`, `is_active`, `search_phrase`, `save_cookie`, `exclude_mobile`, `exclude_countries`, `include_countries`, `data_conditions`) VALUES ('".DBManager::RealEscape($this->Id)."','".DBManager::RealEscape($this->Name)."','".DBManager::RealEscape($this->Created)."','".DBManager::RealEscape($this->Creator)."','".DBManager::RealEscape($this->Edited)."', '".DBManager::RealEscape($this->Editor)."', '".DBManager::RealEscape($this->PagesVisited)."','".DBManager::RealEscape($this->TimeOnSite)."','".DBManager::RealEscape($this->TriggerAmount)."','".DBManager::RealEscape($this->TriggerTime)."',".intval($this->TriggerPage?1:0).",'".DBManager::RealEscape($this->NotDeclined)."', '".DBManager::RealEscape($this->NotAccepted)."',".intval($this->NotInChat ? 1 : 0).",".intval($this->IsInChat ? 1 : 0).",'".DBManager::RealEscape($this->Priority)."', '".DBManager::RealEscape($this->IsActive)."', '".DBManager::RealEscape($this->SearchPhrase)."', '".DBManager::RealEscape(($this->SaveInCookie) ? 1 : 0)."', '".DBManager::RealEscape(($this->ExcludeMobile) ? 1 : 0)."', '".DBManager::RealEscape($this->ExcludeCountries)."', '".DBManager::RealEscape($this->IncludeCountries)."', '".DBManager::RealEscape(serialize($this->DataConditions))."');";
	}

	function GetXML()
	{
		$xml = "<ev id=\"".base64_encode($this->Id)."\" sc=\"".base64_encode($this->SaveInCookie)."\" nacc=\"".base64_encode($this->NotAccepted)."\" ndec=\"".base64_encode($this->NotDeclined)."\" name=\"".base64_encode($this->Name)."\" prio=\"".base64_encode($this->Priority)."\" created=\"".base64_encode($this->Created)."\" nic=\"".base64_encode($this->NotInChat)."\" iic=\"".base64_encode($this->IsInChat)."\" creator=\"".base64_encode($this->Creator)."\" editor=\"".base64_encode($this->Editor)."\" edited=\"".base64_encode($this->Edited)."\" tos=\"".base64_encode($this->TimeOnSite)."\" ta=\"".base64_encode($this->TriggerAmount)."\" tt=\"".base64_encode($this->TriggerTime)."\" tp=\"".base64_encode($this->TriggerPage?1:0)."\" pv=\"".base64_encode($this->PagesVisited)."\" ia=\"".base64_encode($this->IsActive)."\" sp=\"".base64_encode($this->SearchPhrase)."\" em=\"".base64_encode($this->ExcludeMobile ? 1 : 0)."\" ec=\"".base64_encode($this->ExcludeCountries)."\" ic=\"".base64_encode($this->IncludeCountries)."\">\r\n";
...
class ChatArchive
{
    static function GetSearchQuery($_sFor,$_user=false)
    {
        if($_user)
            return "LOWER(`external_id`) LIKE '%".$_sFor."%'";
        return "LOWER(`fullname`) LIKE '%".$_sFor."%' OR LOWER(`area_code`) LIKE '%".$_sFor."%' OR LOWER(`html`) LIKE '%".$_sFor."%'  OR LOWER(`plaintext`) LIKE '%".$_sFor."%' OR LOWER(`transcript_text`) LIKE '%".$_sFor."%' OR LOWER(`email`) LIKE '%".$_sFor."%' OR LOWER(`company`) LIKE '%".$_sFor."%' OR LOWER(`phone`) LIKE '%".$_sFor."%' OR LOWER(`chat_id`) LIKE '%".$_sFor."%' OR LOWER(`external_id`) LIKE '%".$_sFor."%' OR LOWER(`question`) LIKE '%".$_sFor."%'";
    }

    static function GetMatchCount($_sFor)
    {
        $result = DBManager::Execute(true, "SELECT COUNT(*) as `mcount` FROM `" . DB_PREFIX . DATABASE_CHAT_ARCHIVE . "` WHERE closed>0 AND " . ChatArchive::GetSearchQuery($_sFor, true));
        if($result && $row = DBManager::FetchArray($result))
            return $row["mcount"];
        return 0;
    }
            ...
            if(!$_internal->IsBot)
            {
                if(!empty($_chat))
                {
                    ...
                    if(!empty($lm))
                    {
                        ...
                        if(!$wasAutoPost /*|| $lastMessageExternal*/)
                        {
...
class DataInput
{
    ...
    function GetHeight()
    {
        return ($this->Type == "TextArea") ? 120 : 34;
    }
...
class SocialMediaChannel
{
    ...
    function Connect($_data, $_action)
    {
        ...
        $context  = stream_context_create($opts);
        $response = file_get_contents(CONFIG_LIVEZILLA_SOCIAL . strtolower(SocialMediaChannel::GetChannelTypeName($this->Type)) . "/?a=" . $_action, false, $context);
...
class KnowledgeBase
{
    ...
    static function GetTOPEntries($_root="",$_language="")
    {
        $topList = array();
        ...
        $rlist = array();
        if($result = DBManager::Execute(true, "SELECT * FROM `" . DB_PREFIX . DATABASE_RESOURCES . "` WHERE `kb_public`=1 AND `type`!=0 AND `discarded`=0 ORDER BY `title` ASC"))
        {
            while($row = DBManager::FetchArray($result))
            {
                $entry = new KnowledgeBaseEntry($row);
                if(isset($topList[$entry->Id]))
                {
                    if($entry->MatchesLanguage($_language))
                    {
    ...
    static function Process($_userId, $_resId, $_value, $_type, $_title, $_disc, $_parentId, $_size = 0, $_tags = "")
    {
        if($_size == 0)
            $_size = strlen($_title);
    ...
    static function CreateFolders($_owner,$_internal)
    {
        if($_internal)
        {
            KnowledgeBase::Process($_owner, 3, "%%_Files_%%", 0, "%%_Files_%%", 0, 1);
            KnowledgeBase::Process($_owner, 4, "%%_Internal_%%", 0, "%%_Internal_%%", 0, 3);
        }
        else
        {
            KnowledgeBase::Process($_owner, 3, "%%_Files_%%", 0, "%%_Files_%%", 0, 1);
            KnowledgeBase::Process($_owner, 5, "%%_External_%%", 0, "%%_External_%%", 0, 3);
        }
    }
    ...
    static function GetExternalURL($_id="",$_absolute=true,$_version="v1")
    {
        $params = KnowledgeBase::GetAllowedParameters("get",array(0=>"search-for"));
        $key = ($_version=="v1") ? "id" : "article";
        ...
        {
            if($_absolute)
                $url = LIVEZILLA_URL . "knowledgebase.php?e=MQ_";
            else
                $url = "./knowledgebase.php?e=MQ_";
        }
        else
            $url = "./knowledgebase.php?entry=show";

        if(isset($_GET["search-for"]))
            $url .= "&search-for=" . urlencode($_GET["search-for"]);
    ...
    static function GetAllowedParameters($_type="get",$_exclude=null)
    {
        $allowed = array("tops"=>true,"e"=>true,"el"=>true,"epc"=>true,"etc"=>true,"ckf"=>true,"no-logo"=>true,"search-for"=>true);
    ...
    {
        $files = array();
        if($result = DBManager::Execute(true, "SELECT * FROM `" . DB_PREFIX . DATABASE_RESOURCES . "` WHERE `created`>(UNIX_TIMESTAMP()-86400) AND `parentid`=100 AND `discarded`=0 AND `type`=3 AND `editor`='". DBManager::RealEscape($_visitorId."_".$_fIndex) . "' LIMIT 10;"))
            while($row = DBManager::FetchArray($result))
    ...
    {
        DBManager::Execute(true, "UPDATE `" . DB_PREFIX . DATABASE_RESOURCES . "` SET `discarded`=1 WHERE `created`>(UNIX_TIMESTAMP()-86400) AND `parentid`=100 AND `type`=3 AND `id`='". DBManager::RealEscape($_id) . "' LIMIT 1;");
    }
}
...
class KnowledgeBaseEntry
{
    ...
    function Load($_id)
    {
        if($result = DBManager::Execute(true, "SELECT * FROM `" . DB_PREFIX . DATABASE_RESOURCES . "` WHERE `id` = '" . DBManager::RealEscape($_id) . "'"))
            $this->SetDetails(DBManager::FetchArray($result));
    }
    ...
    {
        if(!empty($_id) && KnowledgeBaseEntry::GetById($_id) == null)
        {
    ...
    function GetHTML($_color,$_inChat=true,$_lineBreak=true,$_version="v1")
    {
        if($_version=="v2")
        {
            ...
            $html = str_replace("<!--href-->",KnowledgeBase::GetExternalURL(Encoding::Base64UrlEncode($this->Id),true,$_version),$html);
        }
        else
        {
            $html = IOStruct::GetFile(PATH_TEMPLATES . (($this->Type==2 || $this->Type==3 || $this->Type==4) ? "kb_result_link.tpl" : "kb_result_text.tpl"));
            $html = str_replace("<!--href-->",($_inChat) ? "javascript:parent.lz_chat_show_kb_entry('<!--id-->');" : KnowledgeBase::GetExternalURL(Encoding::Base64UrlEncode($this->Id)),$html);
        }
    ...
    {
        $count = 0;
        $content = str_repeat(",". $this->Tags,5);
...
class Feedback
{
    ...
    {
        $result = DBManager::Execute(true, "SELECT COUNT(`id`) AS `fb_count` FROM `" . DB_PREFIX . DATABASE_FEEDBACKS . "` WHERE `created`>" . DBManager::RealEscape(time() - 86400) . " AND `ip_hash`='" . DBManager::RealEscape(md5(Communication::GetIP())) . "';");
        if($result)
        {
            $row = DBManager::FetchArray($result);
...
class UserData extends BaseObject
{
    ...
    function LoadFromPassThru()
    {
        ...
        if(empty($this->Fullname) && !empty($di))
        {
            $this->Fullname = $di;
            $wc = true;
        }
        ...
        if(empty($this->Company) && !empty($di))
        {
            $this->Company = $di;
            $wc = true;
        }
        ...
        foreach(Server::$Inputs as $index => $input)
            if($input->Custom && $input->Active)
            {
                if(!empty($_GET["cf".$index]))
                    $di = Encoding::Base64UrlDecode(getParam("cf".$index));
                else
...
class GeoTracking
{
    ...
    {
        if(isset(Server::$Configuration->File["gl_pr_ngl"]) && !empty(Server::$Configuration->File["gl_pr_ngl"]) && isset(Server::$Configuration->File["gl_use_ngl"]) && !empty(Server::$Configuration->File["gl_use_ngl"]))
        {
            $_toReplace = str_replace("<!--geo_url-->",base64_encode(CONFIG_LIVEZILLA_GEO_PREMIUM . "?aid=1&sid=".base64_encode(Server::$Configuration->File["gl_lzid"])."&dbp=".Server::$Configuration->File["gl_gtdb"]),$_toReplace);
