...
class Operator extends BaseUser
{
    ...
    function ValidateUpdateSession($_token,$_clientSystemId)
    {
        ...
        DBManager::Execute(true, "UPDATE `" . DB_PREFIX . DATABASE_OPERATORS . "` SET `ip`='".DBManager::RealEscape(Communication::GetIP(true))."',`client_system_id`='" . DBManager::RealEscape($this->ClientSystemId) . "',`token`='" . DBManager::RealEscape($this->Token) . "' WHERE `system_id`='" . DBManager::RealEscape($this->SystemId) . "' LIMIT 1;");
    }
...
class Visitor extends BaseUser
{
	...
	{
		$this->FirstCall = ($_data["closed"] > 0 && !$this->ExtendSession);
    ...
    function GetEntranceTime()
    {
        ...
        foreach($this->Browsers as $browser)
            $t = min($t,$browser->History[0]->Entrance);
        return $t;
    }
	...
	function AddFunctionCall($_call,$_overwrite=false,$_prepend=false)
	{
		if(empty($this->Response))
			$this->Response = "";
        if($_prepend)
            $this->Response = $_call . $this->Response;
		else if($_overwrite)
			$this->Response = $_call;
		else
			$this->Response .= $_call;
...
class VisitorChat extends VisitorBrowser
{
    ...
    function SetTargetOperatorId($_router,$_user,$_allowBots=false,$_requireBot=false,$_exclude=null,$_closeOnError=true,$_isForward=false,$_botmode=false)
    {
        ...
        {
            $_user->AddFunctionCall("lz_chat_input_bot_state(false,false);",false);
