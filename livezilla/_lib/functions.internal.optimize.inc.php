...
{
    static function Optimize($_table)
    {
    ...
    static function Maintain()
    {
        ...
        if(!STATS_ACTIVE)
        {
            DBManager::Execute(true, "DELETE FROM `" . DB_PREFIX . DATABASE_VISITORS . "` WHERE `closed`<" . DBManager::RealEscape(time() - (Server::$Configuration->File["gl_dvhd"] * 86400)) . ";");
        ...
        DBManager::Execute(true, "DELETE FROM `" . DB_PREFIX . DATABASE_VISITOR_CHATS . "` WHERE `exit` > 0 AND `last_active` < " . intval(time() - Server::$Configuration->File["timeout_track"]) . " AND NOT EXISTS (SELECT * FROM `" . DB_PREFIX . DATABASE_VISITORS . "` WHERE `id` = `" . DB_PREFIX . DATABASE_VISITOR_CHATS . "`.`visitor_id`)");
        ...
        if($result = DBManager::Execute(true, "SELECT `value` FROM `" . DB_PREFIX . DATABASE_RESOURCES . "` WHERE `discarded`=1 AND `type`>2"))
            while($result && $row = DBManager::FetchArray($result))
    ...
    static function ApplyTicketAutoStatus()
    {
        ...
        if($result = DBManager::Execute(true, "SELECT * FROM `" . DB_PREFIX . DATABASE_TICKETS . "` WHERE `auto_status_time`>0 AND `auto_status_time`<" . time()))
        {
            while($result && $row = DBManager::FetchArray($result))
            {
                ...
                {
                    $statusUpdates = true;
                    $ticket->Log(0,"SYSTEM",$row["auto_status_status"],$ticket->Editor->Status);
