                                         ...
                                         implements Zend_Mail_Storage_Writable_Interface
{
    ...
    {
        // This could fail in the middle of the task, which is not optimal.
    ...
    {
        // This is also not atomar and has similar problems as removeFolder()
    ...
    public function setFlags($id, $flags)
    {
        ...
        // double dirname to make sure we always move to cur. if recent flag has been set (message is in new) it will be moved to cur.
    ...
    protected function _calculateMaildirsize() {
        ...
        foreach ($folders as $folder) {
            ...
            foreach (array('cur', 'new') as $subsubdir) {
                ...
                // we are using mtime instead of "the latest timestamp". The latest would be atime
                // and as we are accessing the directory it would make the whole calculation useless.
                $timestamps[$dirname] = filemtime($dirname);

                $dh = opendir($dirname);
                // Should have been checked in constructor. Not throwing an exception here, quotas will
    ...
    protected function _calculateQuota($forceRecalc = false) {
        ...
        // $maildirsize equals false if it wasn't set (AKA we recalculated) or it's only
