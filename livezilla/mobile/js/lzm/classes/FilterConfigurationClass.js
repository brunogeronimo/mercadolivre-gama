...
FilterConfiguration.prototype.showFilterCreation = function(type, visitor, filter, ticket, inDialog) {
    var headerString = t('Filter'), that = this;
    var visitorId = (visitor != null) ? visitor.id : '';
    var filterId = (filter != null) ? filter.filterid : '';

    if(type == 'edit')
        type = (filter.t=="0") ? 'visitor' : 'email';
    ...
    if(type=='visitor')
        tabArray.push({name: t('Reason'), content: reasonHtml});
...
FilterConfiguration.prototype.createVisitorFilterMainHtml = function(type, visitor, filter, ticket) {
    ...
    var userIpChecked = (visitor != null || (filter != null && filter.ip != '' && filter.ip != '0.0.0.0') || (visitor == null && filter == null)) ? ' checked="checked"' : '';
    ...
    var emailSubjectChecked = (emailSubject != '') ? ' checked="checked"' : '';
    ...
    var filterHtml = '<div class="lzm-fieldset" data-role="none" id="visitor-filter-main">' +
        '<input type="hidden" value="' + filterId + '" id="filter-filterid" />' +
        '<table id="visitor-filter-main-table">' +
        '<tr><td colspan="2">' +
        '<span><input type="checkbox" class="checkbox-custom" data-role="none" id="filter-active" '+filterActive+' />' +
        ...
        '<td>' +
        '<span><input type="text" data-role="none" id="filter-name" value="' + filterName + '" /></span></td>' +
        '</tr><tr class="top-space-half">' +
        '<td style="white-space: nowrap;"><span>' + t('Filter Type:') + '</span></td>' +
        '<td>' +
        '<span><select data-role="none" id="filter-type">' +
    ...
    if(type =='email')
        filterHtml+= '<fieldset style="margin-top: 10px;" class="lzm-fieldset" data-role="none" id="email-filter-base">' +
            '<legend>' + tid('criteria') + '</legend>' +
            '<table id="email-filter-base-table">' +
            '<tr>' +
            '<td style="width: ' + leftColumnWidth + 'px !important; white-space: nowrap;">' +
            '<span><input type="checkbox" class="checkbox-custom" data-role="none" id="filter-email-check"' + emailChecked + ' />' +
            '<label for="filter-email-check" class="checkbox-custom-label">' + tidc('email') + '</label></span></td>' +
            '<td>' +
            '<input type="text" data-role="none" id="filter-email" value="' + emailEmail + '" /></td>' +
            '<td><div class="lzm-info-text left-space">' + t('Use * as wildcard') + '</div></td>' +
            '</tr><tr class="top-space-half"><td>' +
            '<span><input style="vertical-align: middle;" type="checkbox" class="checkbox-custom" data-role="none" id="filter-subject-check"' + emailSubjectChecked + ' />' +
            '<label for="filter-subject-check" class="checkbox-custom-label"' + '>' + tidc('subject') + '</label></span></td>' +
            '<td><span><input type="text" data-role="none" id="filter-subject" value="' + emailSubject + '" /></span></td>' +
            '<td><div class="lzm-info-text left-space">' + t('Use * as wildcard') + '</div></td>' +
            '</tr></table></fieldset>';


    if(type =='visitor')
        filterHtml+= '<fieldset style="margin-top: 10px;" class="lzm-fieldset" data-role="none" id="visitor-filter-base">' +
            '<legend>' + t('Based On') + '</legend>' +
            '<table id="visitor-filter-base-table">' +
            '<tr>' +
            '<td style="width: ' + leftColumnWidth + 'px !important; white-space: nowrap;">' +
            '<span><input style="vertical-align: middle;" type="checkbox" class="checkbox-custom" data-role="none" id="filter-ip-check"' + userIpChecked + ' />' +
            '<label for="filter-ip-check" class="checkbox-custom-label">' + t('IP Address:') + '</label></span><br /></td>' +
            '<td>' +
            '<input type="text" data-role="none" class="lzm-filter-input-main' + visitorIpDisabled + '" id="filter-ip" value="' + visitorIp + '" /></td>' +
            '<td><div class="lzm-info-text left-space">' + t('Use * as wildcard') + '</div></td>' +
            '</tr><tr class="top-space-half">' +
            '<td>' +
            '<span><input style="vertical-align: middle;" type="checkbox" class="checkbox-custom" data-role="none" id="filter-id-check"' + userIdChecked + ' />' +
            '<label for="filter-id-check" class="checkbox-custom-label"' + '>' + t('User ID:') + '</label></span></td>' +
            '<td colspan="2">' +
            '<span><input type="text" data-role="none" class="lzm-filter-input-main' + visitorIdDisabled + '" id="filter-id" value="' + visitorId + '" /></span></td>' +
            '</tr><tr class="top-space-half"><td>' +
            '<span><input style="vertical-align: middle;" type="checkbox" class="checkbox-custom" data-role="none" id="filter-lg-check"' + languagesChecked + ' />' +
            '<label for="filter-lg-check" class="checkbox-custom-label">' + t('Languages:') + '</label></span>' +
            '</td><td>' +
            '<span><input type="text" data-role="none" class="lzm-filter-input-main' + languagesDisabled + '" id="filter-lg" value="' + filterLanguages + '" /></span>' +
            '</td><td><div class="lzm-info-text left-space">' + t('comma separated, <!--link-->', [['<!--link-->', iso639Link]]) + '</div></td>' +
            '</tr><tr class="top-space-half"><td>' +
            '<span><input style="vertical-align: middle;" type="checkbox" class="checkbox-custom" data-role="none" id="filter-co-check"' + countriesChecked + ' />' +
            '<label for="filter-co-check" class="checkbox-custom-label">' + t('Countries:') + '</label></span>' +
            '</td><td>' +
            '<span><input type="text" data-role="none" class="lzm-filter-input-main' + countriesDisabled + '" id="filter-co" value="' + filterCountries + '" />' +
            ...
            '<tr><td><span>' +
            '<input style="vertical-align: middle;" type="checkbox" class="checkbox-custom" data-role="none" id="filter-chat-check"' + appliesChatsChecked + ' />' +
            '<label for="filter-chat-check" class="checkbox-custom-label">' + t('Chats') + '</label>' +
            '</span></td></tr>' +
            '<tr><td><span>' +
            '<input style="vertical-align: middle;" type="checkbox" class="checkbox-custom" data-role="none" id="filter-ticket-check"' + appliesTicketsChecked + ' />' +
            '<label for="filter-ticket-check" class="checkbox-custom-label">' + t('Tickets') + '</label>' +
            '</span></td></tr>' +
            '<tr><td><span>' +
            '<input style="vertical-align: middle;" type="checkbox" class="checkbox-custom" data-role="none" id="filter-monitoring-check"' + appliesMonitoringChecked + ' />' +
...
FilterConfiguration.prototype.createVisitorFilterReasonHtml = function(filter) {
    ...
    var filterHtml = '<div class="lzm-fieldset top-space" data-role="none" id="visitor-filter-reason">' +
        ...
        '</div>' +
        '<input type="text" data-role="none" id="filter-reason" value="' + reason + '" />' +
        '</div>';
    return filterHtml;
};
...
FilterConfiguration.prototype.createVisitorFilterExpirationHtml = function(filter) {
    ...
    var filterHtml = '<div class="lzm-fieldset top-space" data-role="none" id="visitor-filter-expiration">' +
        '<div>' +
        '<input type="checkbox" class="checkbox-custom" data-role="none" id="filter-exp-check" checked="checked" />' +
        '<label for="filter-exp-check" class="checkbox-custom-label">' + t('Expire after') + '</label>' +
        '&nbsp;&nbsp;<input type="text" data-role="none" style="width: 40px;" id="filter-expire-after"' +
...
FilterConfiguration.prototype.showFilterList = function() {
    ...
    lzm_displayHelper.createDialogWindow(headerString, bodyString, footerString, 'filter-list', {}, {}, {}, {}, '', dialogData, true, false, 'filter_list_dialog');
...
FilterConfiguration.prototype.updateFilterList = function() {
    if ($('#filter-list-body').length > 0)
        $('#filter-list-body').html(this.createFilterListHtml());
};
...
FilterConfiguration.prototype.saveFilter = function(type,filterType) {
    ...
    var filter = {creator: lzm_chatDisplay.myId, editor: lzm_chatDisplay.myId, vip: userIp, vid: userId,
        expires: expires, fname: $('#filter-name').val(), freason: filterReason, fid: filterId,
        state: activeCheck, type: type, exertion: $('#filter-type').val(), lang: languages, countries: countries, allow_chats: allowChats,
        allow_tickets: allowTickets, allow_monitoring: allowMonitoring, subject: subject, email:email, filterType:((filterType=='email')?'1':'0')};
