...
ChatSettingsClass.prototype.createSettingsHtml = function() {
    ...
    if (IFManager.IsAppFrame && IFManager.AppOS != IFManager.OS_IOS)
    {
        ...
        if (IFManager.AppOS == IFManager.OS_WINDOWS)
        {
            ...
            generalSettings += '<div class="top-space-double">' + lzm_inputControls.createButton('app-console','','IFManager.IFToggleDevTools()','Developer Tools') + '</div>';
        }
        generalSettings += '</fieldset>';

        // FIND:40 general settings @spellcheck
    }

    if (IFManager.IsDesktopApp() && typeof(window.top.SpellCheck) != 'undefined' && window.top.SpellCheck.AvailableLanguages.length > 0)
    {
        var spellcheckSettings = '<fieldset id="spellcheck-settings" class="lzm-fieldset" data-role="none" style="margin-top: 5px;"><legend>' + tid('spell_check') + '</legend>';

        spellcheckSettings += '<table class="spaced"><tr><td><fieldset class="lzm-fieldset-full">';

        spellcheckSettings += '<legend>' + tid('languages_2') + '</legend>';

        if(window.top.SpellCheck.AvailableLanguages.length > 0){
            var languages = window.top.SpellCheck.AvailableLanguages;
            var langStringArray = [];
            for(i = 0; i < languages.length; i++){

                var langString = lzm_chatDisplay.getLanguageDisplayName(languages[i].substr(0,2)).toString().substr(5) + ', ' + lzm_chatDisplay.getCountryName(languages[i].substr(3)).toString().split('(')[0] + '"' + languages[i] + '"';
                langStringArray.push(langString);
            }
            languages = langStringArray.sort();
            for(i = 0; i < languages.length; i++){
                var isUserLanguage = window.top.SpellCheck.UserLanguages.includes(languages[i].split('"')[1]);
                spellcheckSettings += '<div>' + lzm_inputControls.createCheckbox('toggle-user-Language-' + languages[i].split('"')[1], languages[i].split('"')[0], isUserLanguage,'') + '</div>';
            }
        }else {
            spellcheckSettings += '<div>' + tid('none') + '</div>';
        }

        spellcheckSettings += '</td><td><fieldset class="lzm-fieldset-full">';

        spellcheckSettings += '<legend>' + tid('added_words') + '</legend>';

        if(window.top.SpellCheck.ExcludedWords.length > 0){
            var words = window.top.SpellCheck.ExcludedWords;
            for(i = 0; i < words.length; i++){
                spellcheckSettings += '<div>' + lzm_inputControls.createCheckbox('remove-excluded-word-' + i,words[i],true,'') + '</div>';
            }
        }
        else
        {
        ...
        spellcheckSettings += '</td></tr></table>';
        spellcheckSettings += '</fieldset>';
    }
...
ChatSettingsClass.prototype.createUserManagement = function() {
    ...
    $('#usersettings-menu').css({'display': 'none'});
    $('#minified-dialogs-menu').css('display', 'none');
    var headerString = '<span id="user-management-dialog-headline-text">' + t('User Management (<!--user_count--> Users / <!--group_count--> Groups)',[['<!--user_count-->', DataEngine.operators.getOperatorCount()], ['<!--group_count-->', DataEngine.groups.getGroupCount()]]) +'</span>';
        ...
        }).done(function(data)
        {
            ...
            $(xmlDoc).find('product').each(function()
            {
                ...
                if(!shown && versint > lvers && versint > LocalConfiguration.IgnoreUpdate)
                {
                    ...
                    var security = $(this).attr('critical')=='1' ? '<span class="text-xl text-red">' + tid('yes') + '</span>' : '<span class="text-xl text-gray">' + tid('no') + '</span>';
                    ...
                    updateHTML += '&nbsp;&nbsp;' + '<span>'+tidc('security')+'</span>&nbsp;'+security+'</div>';
...
LocalConfiguration.ShowVisitorsMap = true;
...
LocalConfiguration.Load = function() {
    ...
    LocalConfiguration.ShowVisitorsMap = LocalConfiguration.LoadValue('show_map_',1)!=0;
    ...
    LocalConfiguration.VisitorFilterHideInactive = LocalConfiguration.LoadValue('vf_hideia_',0);
...
LocalConfiguration.Save = function() {
    ...
    LocalConfiguration.SaveValue('show_map_', LocalConfiguration.ShowVisitorsMap ? 1 : 0);
    ...
    LocalConfiguration.SaveValue('vf_hideia_', LocalConfiguration.VisitorFilterHideInactive);
