    ...
    if (chatObj != null)
    {
        if (lzm_commonPermissions.checkUserPermissions('', 'chats', 'forward', {}))
        {
            ...
            for (var key in lzm_chatDisplay.StoredDialogs)
            {
                if (lzm_chatDisplay.StoredDialogs.hasOwnProperty(key)) {
                    if (lzm_chatDisplay.StoredDialogs[key].type == 'operator-invitation' &&
                        ...
                        lzm_chatDisplay.StoredDialogs[key].data['visitor-id'] == chatObj.v + '~' + chatObj.b) {
                        storedForwardId = key;
                    }
                }
            }
            ...
            else
            {
                ...
                {
                    //saveChatInput(chatObj.SystemId);
                    //removeEditor();
                }
...
ChatForwardInvite.prototype.createOperatorForwardInviteHtml = function (type, _chatObj) {
    ...
    $('#cancel-operator-forward-selection').click(function() {
        ...
        if (lzm_chatDisplay.selected_view == 'mychats')
        {
            //var myText = loadChatInput(that.chatObject.SystemId);
            //initEditor(myText, 'CancelFilterCreation', that.chatObject.SystemId);
        }
    });
