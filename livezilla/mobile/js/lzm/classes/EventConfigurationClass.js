...
function EventConfiguration() {
    ...
    this.m_Definitions = [
        {name:'Conditions',conditions:[
            ...
            {name:'IsInChat',target:'visitor', pkey:'vw',xkey:'iic',title:tidc('ev_is_in_chat')},
        ...
        {name:'Actions',actions:[
            ...
            //{name:'ExternalAlert',target:'visitor', editable: true, title:tidc('ev_action_3'), type:'string', value:''},
            //{name:'ExternalWebsitePush',target:'visitor', editable: true, custom:true,title:tidc('ev_action_4')},
            //{name:'ExternalOverlayBox',target:'visitor', editable: true, custom:true,title:tidc('ev_action_5')},
...
EventConfiguration.prototype.showEventConfiguration = function() {
    ...
    var footerString =
        lzm_inputControls.createButton('ec-add-btn', '', '', tid('add'), '', 'force-text',{'margin-left': '4px','padding': '3px 10px'},'',30,'d') +
        lzm_inputControls.createButton('ec-close-btn', '', '', tid('close'), '', 'lr',{'margin-left': '4px'},'',30,'d');

    var dialogData = {};
    var bodyString = this.createEventListHtml();
    lzm_displayHelper.createDialogWindow(headerString, bodyString, footerString, 'event-configuration', {}, {}, {}, {}, '', dialogData, false, false, 'event_configuration_dialog');
    $('#ec-close-btn').click(function() {
        lzm_displayHelper.removeDialogWindow('event-configuration');
    });
...
EventConfiguration.prototype.getPostDataObject = function(remove) {
    ...
    else
    {
        ...
        for(key in this.m_SelectedEvent.Actions)
        {
            ...
            if(action.name == 'ExternalInvitation'){
...
EventConfiguration.prototype.showEventCreationForm = function(type,id) {
    ...
    lzm_displayHelper.minimizeDialogWindow('event_configuration_dialog', 'event-configuration', {}, lzm_chatDisplay.selected_view, false);
    lzm_displayHelper.createDialogWindow(headerString, bodyString, footerString, 'edit-event', {}, {}, {}, {}, '', dialogData, false, false);
    ...
    lzm_displayHelper.createTabControl('event-placeholder', tabArray);

    this.setAdvancedEvents();

    $('#cancel-event').click(function() {
        lzm_displayHelper.removeDialogWindow('edit-event');
        lzm_displayHelper.maximizeDialogWindow('event_configuration_dialog');
    });
    $('#save-event').click(function() {

        if(lzm_commonPermissions.permissions.events=='1' && that.m_SelectedEvent.creator != '' && that.m_SelectedEvent.creator != DataEngine.myId)
...
EventConfiguration.prototype.showEventSubElementCreation = function(action,type,id){
    ...
    var dHtml = '<fieldset class="lzm-fieldset-full" style="width:450px;min-height:50px;"><legend>'+tid('type')+'</legend>'+lzm_inputControls.createSelect('select-element-'+type,selectClass,'','','','','',options,toSelect,'')+'</fieldset>';
    dHtml += '<fieldset id="ev-sub-element-settings" class="lzm-fieldset-full" style="width:450px;min-height:390px;"><legend>'+tid('settings')+'</legend><div id="element-configuration"></div></fieldset>';
    ...
    elementSelect.change(function(){
        ...
        else
        {
            ...
            if(d(def.operatorList)){
                ...
                html += '<label class="top-space-double">'+tidc('operators')+'</label><div class="border-s" style="height:260px;overflow:auto;">' + lzm_inputControls.createOperatorList(def.name+'-receivers',false,false,recList) + '</div>';
            }
...
EventConfiguration.prototype.applyRow = function(action,subType,elementType,obj,objId,def){
    ...
    if(subType=='actions'){
        ...
        if(elementType == 'ExternalInvitation'){
...
EventConfiguration.prototype.createSubElementCustomHtml = function(definition,objId){
    ...
    if(definition.name == 'URLCondition'){
        chtml += '';
        chtml += '<div>' + lzm_inputControls.createCheckbox(prefix + 'cb-url','URL:',false,'') + '</div>';
        chtml += '<div class="left-space-child">' + lzm_inputControls.createInput(prefix + 's-url', '', '', '', '', 'text', '') + '</div><div class="lzm-info-text text-s left-space-child top-space-half">'+tidc('use_as_wildcard',', ')+tidc('example')+' http*mywebsite*/contact*</div>';
        chtml += '<div class="top-space-double">' + lzm_inputControls.createCheckbox(prefix + 'cb-referrer','Referrer:',false,'') + '</div>';
        chtml += '<div class="left-space-child">' + lzm_inputControls.createInput(prefix + 's-referrer', '', '', '', '', 'text', '') + '</div><div class="lzm-info-text text-s left-space-child top-space-half">'+tidc('use_as_wildcard',', ')+tidc('example')+' http*refwebsite*</div>';
        chtml += '<div class="top-space-double">' + lzm_inputControls.createCheckbox(prefix + 'cb-tos',tidc('ev_time_on_site'),false,'') + '</div>';
        chtml += '<div class="left-space-child">' + lzm_inputControls.createInput(prefix + 'int-tos', '', 0, '', '', 'number','','',tid('seconds')) + '</div>';
...
EventConfiguration.prototype.applySubElementCustomLogic = function(definition,obj,objId){
    ...
    if(definition.name == 'DataCondition' && obj != null){
        ...
        var ithtml = '<div class="top-space-double">' + lzm_inputControls.createRadio(prefix + 'r-ist','','inv-text',tid('ac_def_text')) + '</div>';
        ithtml += '<div class="top-space">' + lzm_inputControls.createRadio(prefix + 'r-ict','','inv-text',tidc('custom_text')) + '</div>';
        ithtml += '<div class="left-space-child">' + lzm_inputControls.createArea(prefix + 'a-ict',cText,'','','height:200px;') + '</div>';

        var groupList = [], groups = DataEngine.groups.getGroupList('id',true,false);
        for (i=0; i<groups.length; i++)
            groupList.push({value: groups[i].id, text: groups[i].id});

        var ishtml = '<div class="top-space-double">' + lzm_inputControls.createRadio(prefix + 'r-isgr','','sender-type',tidc('inv_send_group')) + '</div>';
        ...
        ishtml += '<div class="top-space-half left-space-child border-s" style="height:220px;overflow:auto;">' + lzm_inputControls.createOperatorList(prefix + 'inv-senders') + '</div>';

        var tabs = [
            {name: tid('invitation_text'), content: ithtml, hash: 'it'},
            {name: tid('operators'), content: ishtml, hash: 'is'}];

        lzm_displayHelper.createTabControl('ExternalInvitation-placeholder',tabs,0);
        ...
        lzm_displayHelper.createTabControl('ExternalOverlayBox-placeholder',tabs,0);
        ...
        var askAllow = (obj != null && obj.Pushs.length && obj.Pushs[0].ask=='1') || obj == null;
        var targetUrl = (obj != null && obj.Pushs.length) ? obj.Pushs[0].url : 'http://';

        var pHtml = '<div class="top-space">' + lzm_inputControls.createInput(prefix + 's-url', '', targetUrl, tidc('target_url'), '', 'text', '') + '</div>';
        pHtml += '<div class="top-space-half lzm-info-text text-s">' + tidc('example',': ') + ' http://www.website.domain/target.html</div>';
        pHtml += '<div class="top-space-double">' + lzm_inputControls.createCheckbox(prefix + 'cb-ask',tid('ask_allow'),false,'') + '</div>';
        pHtml += '<div class="top-space left-space-child">' + lzm_inputControls.createRadio(prefix + 'r-wpst','wp-tg','wp-text',tid('ac_def_text')) + '</div>';
        pHtml += '<div class="top-space left-space-child">' + lzm_inputControls.createRadio(prefix + 'r-wpct','wp-tg','wp-text',tidc('custom_text')) + '</div>';
        pHtml += '<div class="left-space-child"><div class="left-space-child">' + lzm_inputControls.createArea(prefix + 'a-wpct',cText,'wp-tg','','height:145px;') + '</div></div>';

        var ishtml = '<div class="top-space-half border-s" style="height:330px;overflow:auto;">' + lzm_inputControls.createOperatorList(prefix + 'wp-senders') + '</div>';

        var tabs = [
            {name: tid('website_push'), content: pHtml, hash: 'it'},
            {name: tid('operators'), content: ishtml, hash: 'is'}];

        lzm_displayHelper.createTabControl('ExternalWebsitePush-placeholder',tabs,0);

        if(obj != null)
            loadSenders(obj.Pushs[0],'wp');

        if(!cText.length)
            $('#'+ prefix + 'r-wpst').prop('checked',true);
        else
            $('#'+ prefix + 'r-wpct').prop('checked',true);

        $('#'+ prefix + 'cb-ask').prop('checked',askAllow);

        $('.wp-text').change(function(){
            if($('#'+ prefix + 'r-wpst').prop('checked'))
                $('#'+ prefix + 'a-wpct').addClass('ui-disabled');
            else
                $('#'+ prefix + 'a-wpct').removeClass('ui-disabled');
        });

        $('#'+ prefix + 'cb-ask').change(function(){
            if(!$('#'+ prefix + 'cb-ask').prop('checked'))
                $('.wp-tg').addClass('ui-disabled');
            else
                $('.wp-tg').removeClass('ui-disabled');
        });


        $('#'+ prefix + 'cb-ask').change();
        $('.wp-text').change();
...
EventConfiguration.prototype.createEventHtml = function(tab){
    ...
    if(tab==0)
    {
        thtml = '<div class="lzm-fieldset">' +
            '<table class="top-space"><tr><td style="padding-top:18px;width:90px;">' + lzm_inputControls.createCheckbox('cb-event-active',tid('active'),(this.m_SelectedEvent.ia=='1'),'') + '</td><td>' + lzm_inputControls.createInput('s-event-name', '', this.m_SelectedEvent.name, tidc('name'), '', 'text', '') +'</td>' +
            '<td>' + lzm_inputControls.createPriorityList('event-priority','',this.m_SelectedEvent.prio,100,tidc('priority')) +'</td></tr></table>';
        thtml += '<table class="top-space">' +
            '<tr><td style="width:50%;vertical-align: top;">'+this.createConditionsHtml()+'</td><td></td><td style="width:49%;vertical-align: top;">'+this.createActionsHtml()+'</td></tr>' +
            '</table></div>';
    }
    else if(tab==1)
    {
        thtml = '<div class="lzm-fieldset"><fieldset class="lzm-fieldset-full top-space-half"><legend>'+tid('triggering')+'</legend>';
        ...
        thtml += lzm_inputControls.createInput('s-event-jscript-command','', 'lz_event_fire(\''+this.m_SelectedEvent.id+'\');', tidc('jscript_command'), '', 'text', '');
        thtml += '</fieldset></div>';
    }
    else if(tab==2)
    {
        thtml += '<div class="lzm-fieldset"><fieldset class="lzm-fieldset-full top-space-half"><legend>'+tid('goals')+'</legend>';
        ...
        for(kg in DataEngine.global_configuration.database['goals'])
        {
            ...
            thtml += '<tr><td style="width:25px;">'+lzm_inputControls.createCheckbox('cb-event-goal-' + goal.id,'',checked)+'</td><td>'+goal.title+'</td>' +
                ...
                lzm_inputControls.createButton('ev-goal-'+goal.id+'-remove','','EventConfiguration.__RemoveGoal(\''+goal.id+'\');', tid('remove'), '<i class="fa fa-trash icon-small"></i>', '', {'margin': '0 6px 0 0'}, '', 30, 'b') +
                '</td></tr>';
        }
...
EventConfiguration.prototype.createConditionsHtml = function() {
    var conditionsHtml = '<fieldset class="event-list-table lzm-fieldset-full"><legend>'+tid('conditions')+'</legend>' +
        '<div class="lzm-dialog-headline3 text-xxl text-green">'+tid('if').toUpperCase()+' ...</div>';
...
EventConfiguration.prototype.createActionsHtml = function() {
    var actionsHtml = '<fieldset class="event-list-table lzm-fieldset-full"><legend>'+tid('actions')+'</legend>' +
        '<div class="lzm-dialog-headline3 text-xxl text-green">... '+tid('then').toUpperCase()+'</div>';
...
EventConfiguration.prototype.createEventSubTableHtml = function(type) {
    ...
    var tableHtml = '<table class="visible-list-table alternating-rows-table lzm-unselectable" id="events-'+type+'-table"><thead><tr><th colspan="2">' + tid(type) + '</th></tr></thead><tbody>';
    var addButtonClass = '';
    ...
    if(type=='conditions')
    {
        ...
        tableHtml += (ctableHtml.length) ? ctableHtml : '<tr><td colspan="2" class="text-center"><i>'+tid('none')+'<br>('+tid('fire_unc')+')</i></td></tr>';
    }
    else
    {
        var atableHtml = this.createEventSubActions();
        tableHtml += (atableHtml.length) ? atableHtml : '<tr><td colspan="2" class="text-center"><i>'+tid('none')+'<br>('+tid('nothing_happen')+')</i></td></tr>';
    ...
    tableHtml += '<hr><div style="text-align:right;padding:8px 0;">'+lzm_inputControls.createButton('add-' + type, addButtonClass, 'showEventSubElementCreation(\'add\',\''+type+'\',\'\',null);', tid('add'), '', 'lr',{'margin-left': '4px'},'',30,'d');
    return '</div>' + tableHtml;
};
...
EventConfiguration.prototype.createEventSubActions = function() {
    ...
    for(var key in this.m_SelectedEvent.Actions)
    {
...
EventConfiguration.prototype.createEventSubRowHtml = function(obj,name,title,type,id,edit) {
    ...
    if(edit)
        rowHtml += lzm_inputControls.createButton('edit-' + fid, '', 'showEventSubElementCreation(\'edit\',\''+type+'\',\''+fid+'\');', '', '<i class="fa fa-gear icon-small"></i>', 'lr',{'margin-left': '4px'},'',30,'');
    rowHtml += lzm_inputControls.createButton('remove-' + fid, '', 'showEventSubElementCreation(\'remove\',\''+type+'\',\''+fid+'\');', '', '<i class="fa fa-trash icon-small"></i>', 'lr',{'margin-left': '4px'},'',30,'')+'</td></tr>';
    return rowHtml;
};
