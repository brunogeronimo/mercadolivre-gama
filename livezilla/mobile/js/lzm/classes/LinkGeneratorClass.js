...
LinkGeneratorClass.prototype.ShowLinkGenerator = function() {
    ...
    var footerString =
        ...
        lzm_inputControls.createButton('get-code-btn', disabledClass, 'showLinkGeneratorCode()', tid('get_code'), '<i class="fa fa-code"></i>', 'force-text',{'margin-left': '4px','padding': '3px 10px'},'',30,'d') +
    ...
    lzm_displayHelper.createDialogWindow(headerString, bodyString, footerString, 'link-generator', {}, {}, {}, {}, '', dialogData, true, true, 'link_generator_dialog');
...
LinkGeneratorClass.prototype.SelectElementType = function(editObj) {
    ...
    var footerString =
        ...
        lzm_inputControls.createButton('cancel-element-type-btn', '', '', t('Cancel'), '', 'lr', {'margin-left': '6px'},'',30,'d');
...
LinkGeneratorClass.prototype.AddLinkGeneratorElement = function(elementType, editObj) {
    ...
    var footerString =
        lzm_inputControls.createButton('save-element-btn', '', '', t('Save'), '', 'lr',{'margin-left': '6px'},'',30,'d') +
        lzm_inputControls.createButton('cancel-element-btn', '', '', t('Cancel'), '', 'lr', {'margin-left': '6px'},'',30,'d');
    ...
    else if(element.m_Type == 'overlay-widget-v2')
    {
        ...
        if(editObj == null)
        {
            UIRenderer.getSettingsProperty(element.m_Settings,'m_MarginRight').value =
...
LinkGeneratorClass.prototype.PreviewInline = function(_cid){
    var that=this,purl = CommunicationEngine.chosenProfile.server_protocol + CommunicationEngine.chosenProfile.server_url + '/preview.php?inline=1&id=' + _cid + '&t=' + lz_global_timestamp();
...
LinkGeneratorClass.prototype.SaveCodeToServer = function(preview){
    var that = this;
    var data = {};
    data.p_cc_c = lz_global_base64_encode(that.GetCode());
...
LinkGeneratorClass.prototype.GetCodeRow = function(id,objB64,name) {
    ...
    return '<tr id="cl-'+id+'" class="lg-cl-code" onClick="LinkGeneratorClass.__LoadCode(\''+id+'\');" data-name="'+lz_global_base64_encode(name)+'" data-obj="'+objB64+'"><td style="width:20px;padding:4px;" class="text-center">'+icon+'</td><td'+nameClass+'>'+name+'</td><td style="text-align: right;">' + lzm_inputControls.createButton('delete-element-from-server-btn-' + id, '', 'deleteLinkGeneratorCode(\''+id+'\')', '', '<i class="fa fa-trash" style="color:#808080 !important;"></i>', 'lr',{'margin-right': '0', 'padding-left': '8px', 'padding-right': '8px'}, '', 20, 'b') + '</td></tr>'
};
...
LinkGeneratorClass.prototype.GetCode = function(){
    ...
    try
    {
        ...
        /*
            ...
            code += '<!--hide_element_open--><a href="javascript:lz_tracking_init_floating_button_selector([<!--selector_params-->]);" class="<!--class-->"><img <!--img_id-->src="<!--server_image-->?id=<!--image_set-->&type=overlay" width="<!--image_set_width-->" height="<!--image_set_height-->" style="border:0px;" alt="LiveZilla Live Chat Software"></a><!--hide_element_close-->';
...
LinkGeneratorClass.prototype.GetCodeParameters = function(type,element,bind){
    ...
    if(type == 'monitoring' && (this.isTypeSelected('overlay-widget-v1')||this.isTypeSelected('overlay-widget-v2'))){
        ...
        if((!this.isTypeSelected('overlay-widget-v2') && speElem.GetProperty('m_MarginRight').value>0) || (this.isTypeSelected('overlay-widget-v2') && speElem.GetProperty('m_MarginRight').value != 30))
    ...
    this.m_Settings = [
        ...
        {name:'Eye Catcher', title: t('Eye-Catcher'), groups: [
            {name: 'EyeCatcher', title: t('Eye-Catcher'), controls: [
                {name:'m_ECUse', type:'bool', value: false, title: t('Eye-Catcher')},
            ...
            {name: 'ECDimensions', class:'m_ECUse', title: t('Dimensions'), controls: [
                {name:'m_ECWidthAuto', type:'bool', value: '', left:'single', title: t('Width:')+' (px)',top:'single'},
                {name:'m_ECWidth', class:'m_ECWidthAuto', type:'int', value: '280', left:'double', title:''},
                {name:'m_ECHeightAuto', type:'bool', value: '', left:'single', title: t('Height:')+' (px)', top:'double'},
                {name:'m_ECHeight', class:'m_ECHeightAuto', type:'int', value: '100', left:'double', title:'', bottom:'single'}
            ]},
            {name: 'ECMargin', class:'m_ECUse', title: tidc('margin'), controls: [
                {name:'m_ECMarginBottom', type:'int', value: '29', title: tidc('bottom'), left:'single',titleright:'px',top:'single'},
                {name:'m_ECMarginRight', type:'int', value: '29', title: tidc('right'),top:'double', left:'single',titleright:'px',not:['overlay-widget-v1'], bottom:'single'}
            ]},
...
LinkGeneratorElement.prototype.ApplyLogicToForm = function(formType) {
    ...
    else if(formType == "Eye Catcher")
    {
        ...
        {
            $('#sl-m_ECTypeOptions').prop('selectedIndex',1);
            $('#sl-m_ECTypeOptions').addClass('ui-disabled');
            $('.m_ECTypeOptionsImage').css({display:'block'});
...
LinkGeneratorElement.prototype.GetCustomForm = function(formType) {
    ...
    if(formType == "Images"){
        ...
        contentHtml += '<input type="text" autocapitalize="off" autocorrect="off" autocomplete="off" id="apim-command-chat" value="lz_chat_overlay_set_mode(\'chat\');" spellchecker="false" readonly="readonly" class="code-box">';
        contentHtml += '<br><br><input type="text" autocapitalize="off" autocorrect="off" autocomplete="off" id="apim-command-ticket" value="lz_chat_overlay_set_mode(\'ticket\');" spellchecker="false" readonly="readonly" class="code-box">';
        contentHtml += '<br><br><input type="text" autocapitalize="off" autocorrect="off" autocomplete="off" id="apim-command-phone" value="lz_chat_overlay_set_mode(\'phone\');" spellchecker="false" readonly="readonly" class="code-box">';
        contentHtml += '<br><br><input type="text" autocapitalize="off" autocorrect="off" autocomplete="off" id="apim-command-knowledgebase" value="lz_chat_overlay_set_mode(\'knowledgebase\');" spellchecker="false" readonly="readonly" class="code-box">';
        contentHtml += '</fieldset>';
    }
    else if(formType == "APIModeExamples"){
        contentHtml = '<input type="text" autocapitalize="off" autocorrect="off" autocomplete="off" style="max-width:800px;" value="<span onclick=&quot;lz_chat_show();lz_chat_overlay_set_mode(\'chat\');\&quot;>Start Chat</span>" spellchecker="false" readonly="readonly" class="code-box">';
        contentHtml += '<br><br><input type="text" autocapitalize="off" autocorrect="off" autocomplete="off" style="max-width:800px;" value="<img onclick=&quot;lz_chat_show();lz_chat_overlay_set_mode(\'ticket\');&quot; src=&quot;./leave_message.png&quot;>" spellchecker="false" readonly="readonly" class="code-box">';
    }
    return contentHtml;
};
