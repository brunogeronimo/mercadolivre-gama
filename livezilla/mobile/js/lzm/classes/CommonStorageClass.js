    ...
    CommunicationEngine.pollServerDiscrete('backup',data).done(function(data) {
        ...
        $(xmlDoc).find('bu').each(function(){
            ...
            $(xmlDoc).find('bu_set').each(function(){
                ...
                console.log("Restore Local Data From Server");
                if(did == 'kbr')that.saveValue('qrd_recently_used_' + myId,lz_global_base64_decode($(this).text()))
                if(did == 'ts')that.saveValue('ticket_salutations_' + myId,lz_global_base64_decode($(this).text()))
            });
        });
...
CommonStorageClass.prototype.loadValue = function(myKey,fallBackValue) {
    ...
    if(typeof fallBackValue != 'undefined' && loadedValue == null)
