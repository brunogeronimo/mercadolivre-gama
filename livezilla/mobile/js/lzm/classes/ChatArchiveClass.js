...
ChatArchiveClass.prototype.showArchivedChat = function(cpId, cpName, chatId, chatType, elementId) {
    ...
    tableString += '</div>' + that.CreateMatchingChats(chatId,[],elementId) + '<fieldset class="lzm-fieldset" data-role="none" id="chat-content-'+elementId+'-inner"><span>' + t('Text') + '</span></fieldset>';
    ...
    UIRenderer.resizeArchivedChat(elementId);
...
ChatArchiveClass.prototype.CreateArchiveListLine = function(aChat, selectedChatId, inDialog, elementId) {
    ...
    var result = (aChat.t == 1) ? (aChat.sr == 0) ? tid('missed') : (aChat.sr == 1) ? t('Accepted') : t('Declined') : '-';
    var endedBy = (aChat.t == 1) ? (aChat.er == 0) ? tid('user') : tid('operator') : '-';
