...
ChatUI.prototype.CreateChatList = function() {
    ...
    $('#chat-allchats').html('<div id="chat-allchats-headline" class="lzm-dialog-headline2" style="top:0;"></div>' +
        '<div id="all-chats-tree" style="top:31px;"></div>'+
        '<div id="all-chats-body" style="top:31px;"></div>'+
        '<div id="all-chats-preview" style="top:31px;">' +
    ...
    var buttons = '<span class="left-button-list">'
        + lzm_inputControls.createButton('allchats-tree-switch', '', 'handleAllChatsTree();','', '<i class="fa fa-list-ul"></i>', 'lr',{'margin-left': '4px','margin-right': '0px'}, '', 10)
        + lzm_inputControls.createButton('allchats-create-group-chat', '', 'createPublicGroup();', tid('group_chat_create'), '<i class="fa fa-plus"></i>', 'force-text', {'margin-left':'4px'}, '') + '</span>';

    buttons += '<span class="right-button-list">' +
        lzm_inputControls.createButton('allchats-observer-mode', '', 'switchObserverMode();', 'Observer Mode', '<i class="fa fa-eye"></i>', 'lr', {'margin-right':'4px'}, '') +
        lzm_inputControls.createButton('allchats-open-group-chat', '', '', tid('open'), '', 'force-text', {'margin-right':'4px'}, '') +
        '</span>';
...
ChatUI.prototype.CreateChatTree = function() {
    ...
    for(var key in gl)
    {
        ...
        if(d(group.members))
        {
            ...
            ghtml += '<div id="all-chats-group-'+group.id+'" oncontextmenu="openChatLineContextMenu(\'' + group.id + '\', event);" class="lzm-unselectable" ondblclick="OpenChatTab(\''+group.id+'\');" onclick="ChatUI.__TreeClick(this.id);" style="padding-left:'+paddings[2]+';"><i class="fa fa-group icon-light'+col+'"></i>' + gname + '</div>';
            gc++;
        }
    }
    ...
    if(this.SelectedNode.indexOf('all-chats-group-')===0 && !lzm_chatDisplay.IsFullscreenMode())
    {
        ...
        $('#allchats-open-group-chat').click(function(){
            OpenChatTab(that.SelectedNode.replace('all-chats-group-',''));
        });
    }
    else
...
ChatUI.prototype.UpdateChatList = function(_selectedNodeId) {
    ...
    {
        if (lzm_chatDisplay.selected_view == 'mychats' && ChatManager.ActiveChat == 'LIST')
        {
    ...
    if(this.PreviewChatObj != null)
    {
        ...
        if(prco!=null && prco.Messages.length && (prco.Messages.length != this.PreviewChatObj.Messages.length || !$('#all-chats-preview-inner').html().length))
        {
            ...
            for (i=0; i<prco.Messages.length; i++)
            {
                ...
                cpc = lzm_chatDisplay.CreatePostHTML(cpc,false);
...
ChatUI.prototype.CreateOperatorChatLines = function(){
    ...
    if(group != null && d(group.members))
    {
        for(var key in group.members)
        {
            ...
            if(operator != null)
            {
                html += '<tr oncontextmenu="openChatLineContextMenu(\'' + group.id + '\', event);" class="allchats-operator-line"><td></td><td><div style="background-image:url(\'' + lzm_commonConfig.lz_user_states[operator.status].icon + '\');"></div></td><td colspan="'+(this.columnCount+1)+'">'+operator.name+'</td></tr>';
                c++;
            }
        }
    }
...
ChatUI.prototype.CreateVisitorChatLines = function(_countsOnly){
    ...
    var curChat,linesHtml = '',linesMy='',linesClosed='',linesGroup='',linesActive='',linesWaiting='',linesMissed='',linesQueued='',i,lsa='',lsm='',lsmy='';
    ...
    for(var key in DataEngine.ChatManager.Chats)
    {
        ...
        if(ichat.Type != Chat.Visitor && ichat.TabOpen)
            ChatManager.Counts.Internal++;
    }
    ...
    for (i=0; i<list.length; i++)
    {
        ...
        {
            if(!_countsOnly)
                linesMy += this.CreateListLine(curChat, true);
            ChatManager.Counts.My++;
        }
        ...
        if(curChat.IsMissed())
        {
            ChatManager.Counts.Missed++;
            if(!_countsOnly)
                linesMissed += this.CreateListLine(curChat, false);
        }
        ...
        else if(curChat.GetStatus() == Chat.Queue)
        {
            ChatManager.Counts.Queued++;
            if(!_countsOnly)
                linesQueued += this.CreateListLine(curChat, false);
        }
        ...
        else if(curChat.GetStatus() == Chat.Closed)
        {
            ChatManager.Counts.Closed++;
            if(!_countsOnly)
                linesClosed += this.CreateListLine(curChat, false);
        }
    }
...
ChatUI.prototype.CreateListLine = function (_chat) {
    ...
    var onclickAction = ' onclick="' + (((!IFManager.IsAppFrame && !IFManager.IsMobileOS) || IFManager.IsDesktopApp()) ? 'ChatUI.__RowClick(this,\'' + _chat.i + '\');' : 'openChatLineContextMenu(\'' + _chat.i +idsuffix+ '\', event);')+'"';

    if(igc)
        operators = '-';

    var oncontextmenuAction = (!IFManager.IsMobileOS) ? ' oncontextmenu="openChatLineContextMenu(\'' + _chat.i + idsuffix + '\', event);"' : '';
    ...
    var lineHtml = '<tr class="allchats-line'+missedClass+'" id="allchats-line-' + _chat.i + idsuffix + '"' + onclickAction + oncontextmenuAction + ' style="cursor: pointer;">' + that.GetButtonField(_chat)+that.GetIconField(_chat,missed,closed);
    for (i=0; i<LocalConfiguration.TableColumns.allchats.length; i++) {
...
ChatUI.prototype.GetIconField = function(_chat,_missed,_closed) {
...
ChatUI.prototype.GetButtonField = function(_chat) {
    ...
    if(_chat.IsMissed() || _chat.GetStatus()==Chat.Closed)
        button = lzm_inputControls.createButton('ac-create-ticket-'+_chat.i, '', 'showTicketDetails(\'\',false,\'\',\''+_chat.i+'\');', tid('create_ticket'), '', 'force-text',{margin:'1px',display:'inline-block',width:'90px',padding:'4px 6px'}, '', 20, 'd');
    else if(_chat.GetChatGroup() != null)
        button = (this.SelectedNode.indexOf('all-chats-group-')===0) ? '' : lzm_inputControls.createButton('ac-ban-chat-'+_chat.i, '', 'ChatUI.__ListButtonClick(this,\''+_chat.SystemId+'\',\'filter\',event);', tid('ban'), '', 'force-text',{margin:'1px',display:'inline-block',width:'90px',padding:'4px 6px'}, '', 20, 'e');
    else if(_chat.GetStatus() == Chat.Open && _chat.IsMember(DataEngine.myId))
    {
        ...
        button = lzm_inputControls.createButton('ac-accept-chat-'+_chat.i, (_chat.AcceptInitiated ? 'ui-disabled' : ''), 'ChatUI.__ListButtonClick(this,\''+_chat.SystemId+'\',\'accept\',event);', uiText, '', 'force-text',{margin:'1px',display:'inline-block',width:'90px',padding:'4px 6px'}, '', 20, 'd');
    }
    else if(_chat.GetStatus() == Chat.Queue)
        button = lzm_inputControls.createButton('ac-take-chat-'+_chat.i, '', 'ChatUI.__ListButtonClick(this,\''+_chat.SystemId+'\',\'take\',event);', tid('take'), '', 'force-text',{margin:'1px',display:'inline-block',width:'90px',padding:'4px 6px'}, '', 20, 'e');
    else if(_chat.GetStatus() == Chat.Active && _chat.IsHost(DataEngine.myId))
        button = lzm_inputControls.createButton('ac-open-chat-'+_chat.i, '', 'ChatUI.__ListButtonClick(this,\''+_chat.SystemId+'\',\'open\',event);', tid('open'), '', 'force-text',{margin:'1px',display:'inline-block',width:'90px',padding:'4px 6px'}, '', 20, 'e');
...
ChatUI.prototype.CreateChatContextMenu = function(myObject) {
    var disabledClass, onclickAction, contextMenuHtml = '';

    disabledClass = (!myObject.IsHost(DataEngine.myId) && myObject.Type != Chat.ChatGroup) ? ' class="ui-disabled"' : '';
    onclickAction = 'OpenChatTab(\'' + myObject.SystemId + '\');';
    ...
    disabledClass = (myObject.Type != Chat.Visitor) ? ' class="ui-disabled"' : '';
    contextMenuHtml += '<div' + disabledClass + ' onclick="showTicketDetails(\'\', false, \'\', \''+myObject.i+'\');removeChatLineContextMenu();"><span class="cm-line cm-click">' + tid('create_ticket') + '</span></div><hr />';
