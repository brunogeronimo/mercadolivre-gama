...
CommonInputControlsClass.prototype.createInputControlPanel = function(mode, disabledClass) {
    ...
    if((!IFManager.IsAppFrame && !IFManager.IsMobileOS) || IFManager.IsDesktopApp()) {
        panelHtml += lzm_inputControls.createButton('editor-bold-btn', disabledClass, 'lzm_chatInputEditor.bold();', '<span style="font-weight: bold;">B</span>', '', 'lr',{'margin-left': '4px', 'padding-left': '12px', 'padding-right': '12px'}, '', -1) +
            lzm_inputControls.createButton('editor-italic-btn', disabledClass, 'lzm_chatInputEditor.italic();', '<span style="font-style: italic;">I</span>', '', 'lr', {'margin-left': '4px', 'padding-left': '12px', 'padding-right': '12px'}, '', -1) +
            lzm_inputControls.createButton('editor-underline-btn', disabledClass, 'lzm_chatInputEditor.underline();', '<span style="text-decoration: underline;">U</span>', '', 'lr',{'margin-left': '4px', 'padding-left': '12px', 'padding-right': '12px'}, '', -1);
        if (mode == 'basic'){
            panelHtml += lzm_inputControls.createButton('editor-html-btn', '', 'lzm_chatInputEditor.showHTML();', 'HTML', '', 'lr',{'margin-left': '4px', 'padding-left': '12px', 'padding-right': '12px'}, '', -1);
            panelHtml += lzm_inputControls.createButton('editor-add-image-btn', '', 'lzm_chatInputEditor.addImage();', tid('image'), '<i class="fa fa-image"></i>', 'lr',{'margin-left': '4px', 'padding-left': '12px', 'padding-right': '12px'}, '', -1);
            panelHtml += lzm_inputControls.createButton('editor-add-link-btn', '', 'lzm_chatInputEditor.addLink();', tid('link'), '<i class="fa fa-link"></i>', 'lr',{'margin-left': '4px', 'padding-left': '12px', 'padding-right': '12px'}, '', -1);
            panelHtml += lzm_inputControls.createButton('editor-add-placeholder-btn', '', 'lzm_chatInputEditor.addPlaceholder();', tid('add_placeholder'), '<i class="fa fa-percent"></i>', 'lr',{'margin-left': '4px', 'padding-left': '12px', 'padding-right': '12px'}, '', -1);

        }
    }
    if (mode != 'basic')
        panelHtml += lzm_inputControls.createButton('send-qrd', disabledClass, '', '', '<i class="fa fa-database"></i>', 'lr',{'margin-left': '4px'}, tid('knowledgebase'));
    return panelHtml;
};
...
CommonInputControlsClass.prototype.createAvatarField = function(myClass, name, intid){
    ...
    myClass = (d(myClass)) ? ' ' + myClass : '';
    return '<div class="avatar-box'+myClass+'" style="background-image: url(\'./../picture.php' + getParam + '\')"></div>';
};
