function AdminUserManagementClass() {
    ...
    this.DefaultColors = ['ec407a','78909c','0288d1','33691e','0098a6','bf360c','00887a','8c6e63','aa47bc'];
    ...
    this.newOpPerms = '1121202101000210111101111111111111011111011011022';
...
AdminUserManagementClass.prototype.createListView = function(uid, gid, tab) {
    ...
    var panelHtml = lzm_inputControls.createButton('umg-new-btn', '', 'showCreateButtonMenu(event)', t('New'), '<i class="fa fa-plus"></i>', 'lr', {}, t('Create new operator'), 'a') +
        lzm_inputControls.createButton('umg-edit-btn', disabledClass, 'editListObject(null)', t('Edit'), '<i class="fa fa-edit"></i>', 'lr', {'margin-left': '4px'}, t('Edit the selected operator'), 'a') +
        lzm_inputControls.createButton('umg-rm-btn', disabledClass, 'removeListObject(null)', t('Remove'), '<i class="fa fa-remove"></i>', 'lr', {'margin-left': '4px'}, t('Remove the selected operator'), 'a');
...
AdminUserManagementClass.prototype.createOperatorList = function() {
    ...
    for (var i=0; i<operators.length; i++) {
        ...
        icon = '<div class="avatar-box avatar-box-small avatar-box-'+iconcol+'" style="background-image: url(./../picture.php?intid='+lz_global_base64_url_encode(operators[i].id)+'&acid='+lzm_commonTools.guid()+'"></div>';
        opListHtml += '<tr class="operator-list-line lzm-unselectable' + selectedLineClass + '" id="operator-list-line-' + operators[i].id + '"' +
            ' onclick="selectTableLine(\'main\',\'' + operators[i].id + '\');" ondblclick="editListObject(\'' + operators[i].id + '\');"' +
            ' oncontextmenu="showUmgContextMenu(\'' + operators[i].id + '\', event);">' +
            '<td style="text-align:center;">'+icon+'</td>' +
            '<td>' + operators[i].userid + '</td><td>' + operators[i].name + '</td><td>' + opGroups.join(', ') + '</td>' +
            '<td>' + opType + '</td><td>' + opDescription + '</td></tr>';
    }
...
AdminUserManagementClass.prototype.createGroupConfiguration = function(group) {
    ...
    lzm_inputControls.createTabControl('umg-edit-placeholder', tabList, 0, 0);
    lzm_inputControls.createTabControl('gr-input-fields-placeholder', inputFieldsConfData.tabs, 0, $(window).width() - 145);
...
AdminUserManagementClass.prototype.saveUser = function() {
    ...
    var opWebSp = 5;//parseInt($('#permtab-files-size').val());
    if (!$('#permtab-files-can-upload').prop('checked'))
        opWebSp = 0;
    ...
    $('#alert-btn-ok',parent.document).click(function(){
        ...
        if (that.selectedListTab == 'user')
        {
            ...
            if (operator != null && operator.level == 1)
            {
                ...
                lzm_commonDialog.createAlertDialog(alertMessage, [{id: 'ok', name: t('Ok')}],false,false,true);
...
AdminUserManagementClass.prototype.createAccountConfiguration = function(operator, type) {
    ...
    contentHtml += '<div style="margin-top: 10px;"><input type="checkbox" class="checkbox-custom" id="operator-inactive"' + opIsInactive + ' />' + '<label for="operator-inactive" class="checkbox-custom-label">' + t('Deactivate account') + '</label></div>';
...
AdminUserManagementClass.prototype.createGroupsConfiguration = function(operator, type) {
    ...
    var contentHtml = '<div id="op-account-groups" style="padding-top: 1px;">' +
...
AdminUserManagementClass.prototype.createPictureConfiguration = function(operator,isBot) {
    ...
    var pVisible = isBot || true ? 'none' : 'block';
...
AdminUserManagementClass.prototype.createOperatorPermissionsConfiguration = function(operator, selectLists) {
    var that = this;
    var contentHtml = '<div class="op-config-fs" id="op-permissions-configuration">' +
        '<div id="permissions-placeholder"></div></div>';
...
AdminUserManagementClass.prototype.createGeneralPermissionTab = function(operator, perms) {
    ...
    var tabContent = '<fieldset class="lzm-fieldset" id="permtab-general-inner-general" style="height: 110px;">' +
        ...
        '<div><input type="checkbox" class="checkbox-custom" id="permtab-general-use-mobile"' + canUseMobile + ' />' +
        '<label for="permtab-general-use-mobile" class="checkbox-custom-label">' + t('This user is allowed to sign on using the mobile APP or web based client') +
        '</label></div>' +
        '<div style="margin:10px 0 0 0;"><input type="checkbox" class="checkbox-custom" id="permtab-general-use-api"' + canUseApi + ' />' +
        '<label for="permtab-general-use-api" class="checkbox-custom-label">' + t('This user is allowed to make API calls') +
        '</label></div>' +
        '</fieldset>' +
        '<fieldset class="lzm-fieldset" id="permtab-general-inner-admin" style="margin-top: 10px;">' +
        '<legend>' + t('Administrator') + '</legend>' +
        '<div' + adminIsDisabled + ' style="margin: 10px 0px;"><input type="checkbox" class="checkbox-custom" id="permtab-general-is-server-admin"' + isServerAdmin + ' />' +
        '<label for="permtab-general-is-server-admin" class="checkbox-custom-label">' + t('This user is Server Administrator') +
        '</label></div>' +
        '</fieldset>';
...
AdminUserManagementClass.prototype.createFilesPermissionTab = function(operator) {
    ...
    var tabContent = '<fieldset class="lzm-fieldset perm-inner-fs" id="permtab-files-inner">' +
        ...
        /*lzm_inputControls.createInput('permtab-files-size', '', filesSize, t('Webspace max (MB)'), '', 'number', '') +*/
        '</div></fieldset>';
    return tabContent
};
...
AdminUserManagementClass.prototype.createSignatureConfiguration = function(opOrGr) {
    ...
    contentHtml += '</tbody></table></div><div class="top-space" style="margin-bottom: 5px; text-align: right;">';
    ...
    contentHtml += lzm_inputControls.createButton('def-signature-btn', 'ui-disabled sig-edit-btns', 'setSignatureAsDefault()', t('Set as default'), '', 'lr',
        {'padding-left': '12px', 'padding-right': '12px'}, t('Set selected signature as default'), 20, 'b');
    contentHtml += '</div>' +
        '</fieldset>';
...
AdminUserManagementClass.prototype.createSignatureInput = function(signature) {
    ...
    var inputForm = '<div id="signature-inner-div"><fieldset class="lzm-fieldset input-fs" id="signature-inner-fs">' +
        ...
        lzm_inputControls.createInput('signature-name', '', signatureName, t('Name'), '', 'text', '') +
        '' +
        '<label for="signature-text" class="top-space">' + t('Signature') + '</label>' +
        ...
        '<span class="lzm-info-field"><i class="fa fa-info-circle icon-blue icon-large"></i><span>&nbsp;&nbsp;' + t('Please right click on a text field to add a placeholder') + '</span></span>' +
        '</fieldset></div>';
...
AdminUserManagementClass.prototype.createTextAndEmailsConfiguration = function(opOrGr) {
    ...
    var contentHtml = '<fieldset class="lzm-fieldset op-config-fs" id="op-text-emails-configuration">' +
        ...
        '<div id="text-emails-list-div" class="border-s"><table class="visible-list-table alternating-rows-table" id="text-emails-list-table">' +
        '<tbody>';
    ...
    contentHtml += '</tbody></table></div><div class="top-space" style=" margin-bottom: 5px; text-align: right;">';
    contentHtml += lzm_inputControls.createButton('new-text-emails-btn', '', 'createTextEmails()', t('New'), '', 'lr',{'margin-right': '5px', 'padding-left': '12px', 'padding-right': '12px'}, t('Create new texts'), 20, 'b');
    contentHtml += lzm_inputControls.createButton('edit-text-emails-btn', 'ui-disabled text-emails-edit-btns', 'editTextEmails()', t('Edit'), '', 'lr',{'margin-right': '5px', 'padding-left': '12px', 'padding-right': '12px'}, t('Edit selected texts'), 20, 'b');
    contentHtml += lzm_inputControls.createButton('rm-text-emails-btn', 'ui-disabled text-emails-edit-btns', 'removeTextEmails()', t('Remove'), '', 'lr',{'margin-right': '5px', 'padding-left': '12px', 'padding-right': '12px'}, t('Remove selected texts'), 20, 'b');
    contentHtml += lzm_inputControls.createButton('def-text-emails-btn', 'ui-disabled text-emails-edit-btns', 'setTextEmailsAsDefault()', t('Set as default'), '', 'lr',{'padding-left': '12px', 'padding-right': '12px'}, t('Set selected texts as default'), 20, 'b');
    contentHtml += '</div>' +
        '</fieldset>' +
        '<fieldset class="lzm-fieldset op-config-fs" id="op-text-emails-configuration"><legend>' + t('Chats') + '</legend>' +
...
AdminUserManagementClass.prototype.createTextEmailsInput = function(textAndEmails, aw, edit) {
    ...
    var inputForm = '<div id="text-emails-inner-div" data-aw="' + aw + '" data-edit="' + edit + '">' +
        '<div class="top-space bottom-space-double left-space-child">' +
            lzm_inputControls.createSelect('text-emails-lang-select', disabledClass, '', tidc('language'), {position: 'right', gap: '0px'}, {}, '', optionList, selectedLanguage.toLowerCase(), '') +
        '</div><div id="text-emails-edit-placeholder"></div>' +
        '<span class="lzm-info-field left-space-child">' +
        '<i class="fa fa-info-circle icon-blue icon-large"></i><span>&nbsp;&nbsp;' +
...
AdminUserManagementClass.prototype.createTextEmailsTabContent = function(contentArray, textAndEmails) {
    ...
    for (i=0; i<contentArray.length; i++) {
        if (contentArray[i].type == 'textarea') {
            label = '<label for="tae-inner-tab-' + contentArray[i].id + '">' + contentArray[i].label + '</label>';
...
AdminUserManagementClass.prototype.createGroupDetailsConfiguration = function(group) {
    ...
    var contentHtml = '<fieldset class="lzm-fieldset">' +
        '<legend>' + tid('group_details') + '</legend>' +
        lzm_inputControls.createInput('group-id', 'umg-edit-text-input' + idDisabled, groupId, tidc('group_id'), '<i class="fa fa-barcode"></i>', 'text', 'a') +
        lzm_inputControls.createInput('group-email', 'umg-edit-text-input', groupEmail, tidc('group_email'), '<i class="fa fa-envelope-o"></i>', 'text', 'a') +
        '<div class="top-space">' +
        ...
        '</div>' +
        '</fieldset>' +
        '<fieldset class="lzm-fieldset top-space-double">' +
        '<legend>' + t('Titles (visible to all users)') + '</legend>' +
        t('Translate the title of your group into the most common languages.') + t('These titles will be shown to the operators and website visitors depending on their computer or browser localization settings.') +
        '<div id="group-title-div" class="border-s top-space"><table class="visible-list-table alternating-rows-table" id="group-title-table">';
    ...
    contentHtml += '</table></div><div class="top-space" style="margin-bottom: 5px; text-align: right;">';
    contentHtml += lzm_inputControls.createButton('new-title-btn', '', 'createGroupTitle()', t('New'), '', 'lr',{'margin-right': '5px', 'padding-left': '12px', 'padding-right': '12px'}, '', 20, 'b');
    contentHtml += lzm_inputControls.createButton('edit-title-btn', 'ui-disabled title-edit-btns', 'editGroupTitle()', t('Edit'), '', 'lr',{'margin-right': '5px', 'padding-left': '12px', 'padding-right': '12px'}, '', 20, 'b');
    contentHtml += lzm_inputControls.createButton('rm-title-btn', 'ui-disabled title-edit-btns', 'removeGroupTitle()', t('Remove'), '', 'lr',{'padding-left': '12px', 'padding-right': '12px'}, '', 20, 'b');
    contentHtml += '</div>' +
        '</fieldset>';
...
AdminUserManagementClass.prototype.createGroupTitleInput = function(titles, lang) {
    ...
    var inputForm = '<div id="group-title-inner-div"><fieldset class="lzm-fieldset input-fs" id="group-title-inner-fs">' +
        ...
        '<div style="margin: 0px 0px 4px 2px;"><label for="gr-title-lang">' + t('Language') + '</label></div><div>' +
        lzm_inputControls.createSelect('gr-title-lang', langDisabled, '', '', {position: 'right', gap: '0px'}, {}, '', langList, lang, '') +
        '</div></div>' +
        lzm_inputControls.createInput('gr-title-text', 'umg-edit-text-input', titleText, t('Title'), '', 'text', '') + '</fieldset></div>';


    parent.lzm_commonDialog.createAlertDialog(inputForm, [{id: 'gt-ok', name: t('Ok')}, {id: 'gt-cancel', name: t('Cancel')}]);

    $('#gr-title-text',parent.document).focus();

    $('#alert-btn-gt-ok',parent.document).click(function() {
        that.saveGroupTitle();
        $('#alert-btn-gt-cancel',parent.document).click();
    });
    $('#alert-btn-gt-cancel',parent.document).click(function() {
        parent.lzm_commonDialog.removeAlertDialog();
    });
...
AdminUserManagementClass.prototype.saveGroupTitle = function() {
    ...
    titleList[$('#gr-title-lang',parent.document).val()] = $('#gr-title-text',parent.document).val();
...
AdminUserManagementClass.prototype.createGroupTicketsConfiguration = function(group, selectedTab) {
    ...
    var emailTabContent = '<fieldset class="lzm-fieldset" id="gr-tickets-email-out-fs"><legend>' + t('Outgoing Emails') + '</legend>' +
        '<div>' +
        '<label for="gr-tickets-out-lz" class="radio-custom-label">' + tidc('send_emails_lz') + '</label></div>' +
    ...
    emailTabContent += '</tbody></table>' +
        ...
        lzm_inputControls.createSelect('gr-ticket-in-action', '', '', '', {position: 'right', gap: '0px'}, {}, '', inActionList, selInAction, '','',3) +
        '</div></div>' +
        '</fieldset>';
    ...
    var routingTabContent = '<fieldset class="lzm-fieldset" id="gr-tickets-routing-fs"><legend>' + t('Assign Tickets') + '</legend>' +
        ...
        '<table style="width: 100%;" class="visible-list-table alternating-rows-table" id="gr-ticket-assign-list-table">' +
...
AdminUserManagementClass.prototype.createGroupTicketsSocialMediaTab = function(socialMediaList) {
    ...
    socialTabContent += '</tbody></table></div><div class="top-space" style="margin-bottom: 5px; text-align: right;">';
    socialTabContent += lzm_inputControls.createButton('add-social-media-btn', '', 'createSocialMedia()', tid('add'), '', 'lr',{'margin-right': '5px', 'padding-left': '12px', 'padding-right': '12px'}, t('Edit selected Social Media Channel'), 20, 'b');
    socialTabContent += lzm_inputControls.createButton('edit-social-media-btn', 'ui-disabled smc-edit-btns', 'editSocialMedia()', tid('edit'), '', 'lr',{'margin-right': '5px', 'padding-left': '12px', 'padding-right': '12px'}, t('Edit selected Social Media Channel'), 20, 'b');
    socialTabContent += lzm_inputControls.createButton('rm-social-media-btn', 'ui-disabled smc-edit-btns', 'removeSocialMedia()', tid('remove'), '', 'lr',{'margin-right': '5px', 'padding-left': '12px', 'padding-right': '12px'}, t('Remove selected Social Media Channel'), 20, 'b');
...
AdminUserManagementClass.prototype.createGroupMonitoringConfiguration = function(group) {
    ...
    contentHtml += '</table></div><div style="text-align: right;" class="top-space">';
    contentHtml += lzm_inputControls.createButton('new-group-filter-btn', '', 'createGroupFilter()', tid('new'), '', 'lr',{'margin-right': '5px', 'padding-left': '12px', 'padding-right': '12px'}, '', 20, 'b');
    contentHtml += lzm_inputControls.createButton('edit-group-filter-btn', 'ui-disabled filter-edit-btns', 'editGroupFilter()', tid('edit'), '', 'lr',{'margin-right': '5px', 'padding-left': '12px', 'padding-right': '12px'}, '', 20, 'b');
    contentHtml += lzm_inputControls.createButton('rm-group-filter-btn', 'ui-disabled filter-edit-btns', 'removeGroupFilter()', tid('remove'), '', 'lr',{'padding-left': '12px', 'padding-right': '12px'}, '', 20, 'b');
...
AdminUserManagementClass.prototype.createGroupFilter = function(filter) {
    ...
    var inputForm = '<fieldset class="lzm-fieldset">' +
        '<legend>' + tid('filter') + '</legend>' +
...
AdminUserManagementClass.prototype.createGroupHoursConfiguration = function(group) {
    ...
    contentHtml += '</tbody></table></div><div style="text-align: right;" class="top-space">';
    contentHtml += lzm_inputControls.createButton('new-group-oh-btn', '', 'createOpeningHour()', t('Add Hours'), '', 'lr',{'margin-right': '5px', 'padding-left': '12px', 'padding-right': '12px'}, '', 20, 'b');
    contentHtml += lzm_inputControls.createButton('clear-group-oh-btn', '', 'clearOpeningHours()', t('Clear Hours'), '', 'lr',{'margin-right': '5px', 'padding-left': '12px', 'padding-right': '12px'}, '', 20, 'b');
    contentHtml += lzm_inputControls.createButton('rm-group-oh-btn', 'ui-disabled oh-edit-btns', 'removeOpeningHour()', t('Remove'), '', 'lr',{'margin-right': '0px', 'padding-left': '12px', 'padding-right': '12px'}, '', 20, 'b');
    contentHtml += '</div></fieldset>';
    return contentHtml;
};
...
AdminUserManagementClass.prototype.GetRandomColor = function(){
    ...
    if(index==0)
        index = 1;
    else if(index>(this.DefaultColors.length-1))
        index = this.DefaultColors.length-1;
...
AdminUserManagementClass.prototype.createNewObjectMenu = function() {
    ...
    contextMenuHtml += '<div onclick="createListObject(\'bot\');"><span id="umg-new-btn-bot" class="cm-line cm-click">' + t('Chat Bot (Virtual Assistant)') + '</span></div><hr />';
...
AdminUserManagementClass.prototype.createSignatureContextMenuHtml = function() {
    var contextMenuHtml = '';
    contextMenuHtml += '<div class="ui-disabled">' +
        '<span id="sig-add-placeholder" class="cm-line">' +
        tidc('add_placeholder') + '</span></div>';
    contextMenuHtml += '<div onclick="addPlaceholder(\'signature\', \'%operator_name%\');">' +
        '<span id="sig-add-op-name" class="cm-line cm-click">' +
...
AdminUserManagementClass.prototype.createTextAndEmailsContextMenuHtml = function(taIdPrefix, taId) {
    var createPlaceholderMenuEntries = function(plA) {
        ...
        for (var i=0; i<plA.length; i++)
        {
            ...
            plEntry += '<div>' +
                '<span id="tae-add-' + plO.id + '" class="cm-line cm-click" onclick="addPlaceholder(\'' + taIdPrefix + '-' + taId + '\', \'' + plO.pl + '\');">' + plO.text + '</span></div>';
        }
        return plEntry;
    };
    ...
    var contextMenuHtml = '';
    contextMenuHtml += '<div class="ui-disabled"><span id="sig-add-placeholder" class="cm-line"><b>' + tidc('add_placeholder') + '</b></span></div><hr>';
