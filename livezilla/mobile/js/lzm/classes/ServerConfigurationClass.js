...
ServerConfigurationClass.prototype.showServerConfiguration = function() {
    ...
    var that = this;
    var footerString =
            lzm_inputControls.createButton('sc-ok-btn', '', '', tid('ok'), '', 'force-text',{'margin-left': '4px','padding': '3px 10px'},'',30,'d') +
    ...
    lzm_displayHelper.createDialogWindow(headerString, bodyString, footerString, 'server-configuration', {}, {}, {}, {}, '', dialogData, true, true, 'server_configuration_dialog');
    $('#sc-close-btn').click(function() {
        lzm_displayHelper.removeDialogWindow('server-configuration');
    });
...
ServerConfigurationClass.prototype.getPostData = function() {
    ...
    $("#email-acc-outgoing tbody tr").each(function() {
        if($(this).attr("data-obj")){
            ...
            data['p_cfg_es_fw_' + count] = (ea.framework == 'PHP') ? 'PHP' : 'ZEND';
            count++;
        }
    });
...
ServerConfigurationClass.prototype.getEmailAccountForm = function(accountObj) {
    ...
    {
        contentHtml += '<div class="top-space">'+lzm_inputControls.createInput('ea_conf_senderName', '', accountObj.senderName, tid('name'), '', 'text', '')+'</div>';
...
ServerConfigurationClass.prototype.getCriteriaForm = function(criteriaObj){
    ...
    contentHtml += '<div>' + lzm_inputControls.createInput('fb-crit-name','',criteriaObj.name,tid('name'),'','','text') + '</div>';
...
ServerConfigurationClass.prototype.ticketSubAction = function(action, type, id) {
    ...
    if(action == 'remove'){
        ...
        lzm_commonDialog.createAlertDialog(lzm_inputControls.createInput('sc-tsd-new-name','','',tid('name'),'','','text'), [{id: 'tsdok', name: tid('ok')}, {id: 'tsdcancel', name: tid('cancel')}],true,true,true);
    ...
    this.m_Settings = [
        ...
        {name:'Server', title: tid('server'), icon:'server', groups: [
            ...
            {name: 'Filter', title: tid('filter'), controls: [
                ...
                {name:'gl_sfa', standard:true, type:'bool', value: false, title: tid('spam_filter'), top:'single'},
    ...
    if(formType== 'EmailAccounts'){
        ...
        try
        {
            if(licenseframe != null)
                ...
                {
                    var sid = DataEngine.getConfigValue('gl_lzid',true).value;
                    var lobj = lzm_commonTools.phpUnserialize(lz_global_base64_decode(licenseframe.subkeys[subkey].toString()));
