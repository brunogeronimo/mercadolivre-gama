...
ChatPollServerClass.prototype.fillDataObject = function () {
    ...
    if(DataEngine.ChatManager.DataUpdateTime != null)
        this.dataObject.p_ext_cl = DataEngine.ChatManager.DataUpdateTime;
        ...
        if (xmlString != '')
        {
            ...
            if (xmlIsLiveZillaXml)
            {
                if(true)
                {
                    ...
                    // operators
                    var p_int_r = DataEngine.getInternalUsers(xmlDoc);
                    if (p_int_r != '')
                        thisClass.addPropertyToDataObject('p_int_r', p_int_r);

                    // lcheck
                    var opsOnline = DataEngine.operators.GetActiveOperators([lzm_chatDisplay.myId]);
                    ...
                    if (validationError != -1)
                    {
                        if (!CommonUIClass.LicenseMissing && !CommonUIClass.LicenseCheckPassed)
                        {
                            ...
                            lzm_commonDialog.createAlertDialog(alertString, [{id: 'ok', name: tid('logout')},{id: 'ignore', name: tid('ignore')},{id: 'bx', name: tid('upgrade')}]);
                            $('#alert-btn-ok').click(function() {
                                lzm_chatDisplay.askBeforeUnload = false;
                            ...
                            if(LocalConfiguration.LastFeedback < DataEngine.feedbacksMaxCreated)
                            {
                                ...
                                if(LocalConfiguration.NotificationFeedbacks)
                                {
                                    ...
                                    if(IFManager.IsDesktopApp())
                                        IFManager.IFShowNotification(t('LiveZilla'), tid('new_feedback'), 'NONE', fb.UserData.f111, '', 4);
                                    else
                    ...
                    if (DataEngine.myId != '')
                    {
                        ...
                        if (thisClass.qrdRequestTime == 0)
                        {
                            ...
                            if(typeof(LocalConfiguration.UserLanguages) != 'undefined'  && IFManager.IsDesktopApp() && typeof(window.top.SpellCheck) != 'undefined'){
                            	window.top.SpellCheck.UserLanguages = LocalConfiguration.UserLanguages;
                            	IFManager.IFSetUserLanguages();
                            }

                            if(LocalConfiguration.ActiveLanguage  && IFManager.IsDesktopApp() && typeof(window.top.SpellCheck) != 'undefined'){
                              window.top.SpellCheck.SetSpellCheck(LocalConfiguration.ActiveLanguage);
                            }

							if(typeof(LocalConfiguration.ExcludedWords) != 'undefined'  && IFManager.IsDesktopApp() && typeof(window.top.SpellCheck) != 'undefined'){
							  window.top.SpellCheck.ExcludedWords = LocalConfiguration.ExcludedWords;
							  IFManager.IFSetExcludedWords();
							}
                ...
                if (DataEngine.new_usr_p || DataEngine.new_int_u || DataEngine.new_int_d)
                {
                ...
                if (DataEngine.new_usr_p || DataEngine.new_ext_f ||
                    DataEngine.new_ext_u || DataEngine.new_int_u ||
                    DataEngine.new_int_d) {
                    if (thisClass.lzm_chatDisplay.selected_view == 'mychats')
                        updateDisplayMyChats = true;
                }

                if(updateDisplayMyChats)
                    thisClass.lzm_chatDisplay.DrawChat(lzm_chatDisplay.thisUser);
                ...
                for (i=0; i<lzm_chatDisplay.myGroups.length; i++)
                {
                    ...
                    if (d(thisGroup.oh) && thisGroup.oh == '1' && d(thisGroup.external) && thisGroup.external=='1')
                        CommonUIClass.IsExternalAvailable = true;

                    if (d(thisGroup.ohs) && thisGroup.ohs.length)
