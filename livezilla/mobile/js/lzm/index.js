...
var fillStringsFromTranslation = function(selectedIndex, selectedStatus) {
    if (loopCounter > 49 || lzm_commonTranslation.translationArray.length != 0) {
        $('#username-label').html(tidc('username'));
        $('#password-label').html(tidc('password'));
        ...
        $('#login-headline-text').html(t('Please enter your login'));
...
var createLoginForm = function(profileIndex, userStatus) {
    ...
    var loginForm = lzm_inputControls.createInput('username', 'login-input', '', 'Username', '<i class="fa fa-user"></i>', 'text', 'a') +
        lzm_inputControls.createInput('password', 'login-input', '', 'Password', '<i class="fa fa-lock"></i>', 'password', 'a') +
        lzm_displayHelper.createSelect('user_status', '', '', '', false, {}, '', optList, userStatus,'') +
        lzm_inputControls.createButton('login_btn', '', '', t('Log in'), '', 'r', {'padding': '7px 15px'}, '',30, 'd') +
    ...
    if (!runningFromApp) {
        loginForm += '<div style="margin: 3px 0;"><input type="checkbox" class="checkbox-custom" data-role="none" name="save_login" id="save_login"' +
    ...
    loginForm += '<div style="margin: 3px 0;"><input type="checkbox" class="checkbox-custom" data-role="none" name="auto_login" id="auto_login" class="auto_login" style="margin-left: 0; vertical-align: middle;" />' +
        '<label id="auto_login-text" for="auto_login" class="checkbox-custom-label" style="padding-left: 1px;">&nbsp;</label></div>';

    loginForm += '<div><input type="checkbox" class="checkbox-custom" data-role="none" name="ldap_login" id="ldap_login" class="ldap_login" style="margin-left: 0; vertical-align: middle;" />' +
    ...
    if (runningFromApp)
    {
        ...
        loginForm += lzm_displayHelper.createSelect('server_profile_selection', '', '', true, {position: 'right', gap: '0px'}, {width: '120px'}, '', profileSelectList, -1, '');
        loginForm += lzm_inputControls.createButton('configure_btn', '', '', '', '<i class="fa fa-gear"></i>', 'lr', {'padding': '7px 15px'}, '', '');
...
$(document).ready(function () {
    ...
    if (!runningFromApp && (!hasStorage() || !hasCookies())) {
        ...
        var errorHtml = '<div style="position:absolute;width:400px;height:200px;top:50%;left:50%;margin: -150px 0 0 -200px; text-align:center; font-size: 1.4em; text-shadow:1px 1px 1px #fff;">' +
