        ...
        if (sel != null) {
            try {
                ...
                if(ibody.lastChild && ibody.lastChild != null)
                {
                    ...
                    try
                    {
                        //alert(caret + "/" + ibody.lastChild.tagName);
...
function lz_he_getText(_eId){
    if (document.getElementById('wysiwyg' + _eId) != null) {
        if (browserName == "Microsoft Internet Explorer"||browserName == "Opera")
            return document.getElementById('wysiwyg' + _eId).contentWindow.document.body.innerText;
        return document.getElementById('wysiwyg' + _eId).contentWindow.document.body.textContent;
    } else {
        return '';
    }
}
...
function lz_he_insertHTML(_html, _eId){
	insertHTML(_html, _eId);
}
...
var m_DragOverCallBackFunction = null;
var m_DefaultFont = "Verdana";
var m_DefaultFontSize = "2";
...
var Headers = ["P","PRE","H1","H2","H3","H4","H5","H6"];
...
function formatSpecialChar(charname, n, selected){insertHTML(charname, n);}
...
function insertLineBreak(n) {
	if (browserName == "Microsoft Internet Explorer"||browserName == "Opera")
	{
		document.getElementById('wysiwyg' + n).contentWindow.document.selection.createRange().pasteHTML("<div></div>");
	}
	else
	{
