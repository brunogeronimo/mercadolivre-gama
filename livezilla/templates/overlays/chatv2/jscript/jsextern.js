...
var lz_mode_chat_feedback = false;
...
OverlayChatWidgetV2.__FunctionFile = false;
OverlayChatWidgetV2.__FunctionPrint = false;
...
function lz_chat_pop_out(_type,_params){
    ...
    lz_chat_update_chat_ui(false);
    lz_chat_overlay_set_mode('',true);
}
...
function lz_chat_switch_options_table(_forceClose){
    ...
    if(lz_chat_id.length && (lz_chat_status == 1 || lz_chat_status == 2))
    {
        lz_remove_class(document.getElementById('lz_cf_ec'),'disabled');
    }
    else
    {
        lz_add_class(document.getElementById('lz_cf_ec'),'disabled');
    }

    document.getElementById("lz_cf_fu").style.display = OverlayChatWidgetV2.__FunctionFile && lz_chat_id.length && lz_chat_status == 2 ? 'block':'none';
    document.getElementById("lz_cf_pr").style.display = OverlayChatWidgetV2.__FunctionPrint ? 'block':'none';
}
...
function lz_chat_set_host(_systemId,_chatId,_groupId,_groupName,_userid,_lang,_image,_fullname,_functions){
    if(lz_overlay_chat != null)
    {
        ...
        if(_systemId != null)
        {
            ...
            lz_chat_set_group(lz_operator.Group);
        }
        else
        {
        ...
        OverlayChatWidgetV2.__FunctionPrint = !(_functions != null && _functions[2]==0);
        document.getElementById("lz_cf_pr").style.display = OverlayChatWidgetV2.__FunctionPrint ? 'none' : 'block';

        OverlayChatWidgetV2.__FunctionFile = !(_functions != null && _functions[5]==0);
        document.getElementById("lz_cf_fu").style.display = OverlayChatWidgetV2.__FunctionFile ? 'none' : 'block';
...
function lz_chat_update_ui_elements(){
    ...
    document.getElementById("lz_overlay_module_feedback").style.visibility = (lz_mode_chat_feedback) ? "visible" : "hidden";
    ...
    if(!lz_mode_change_details)
    {
        ...
        else if(lz_mode_ticket_feedback || lz_mode_chat_feedback)
            displayDataForm = 'none';
        ...
        else if(lz_session.OVLWMSElem == "chat" && !OverlayChatWidgetV2.__FormConfirmRequired)
        {
            ...
            {
                if(lz_chat_available)
                    displayDataForm = 'none';
            }
        }
    }

    if(displayDataForm != 'none' && !inChat && lz_session.OVLWMSElem == 'chat' && lz_chat_data.InputFieldIndices!=null)
...
function lz_chat_start(){
    ...
    if(lz_chat_status > 0 || (lz_chat_status == 0 && lz_check_missing_inputs(true,'lz_chat_start')))
    {
        ...
        if(lz_chat_status == 0)
        {
            if(document.getElementsByName("form_114")[0].value.length == 0)
                document.getElementsByName("form_114")[0].value = "Hi!";
...
function lz_chat_change_group(_box,_control){
...
function lz_chat_set_input_fields(){
    try
    {
        if(lz_chat_data.InputFieldIndices != null && lz_chat_data.SelectedGroup != null)
        {
            ...
            if(isChat && lz_chat_data.SelectedGroup.Amount == 0 && !OverlayChatWidgetV2.__PublicGroupChat.length && lz_chat_status == 0)
            {
                lz_chat_overlay_set_mode("ticket",false);
                return;
            }
            else if(lz_session.OVLWMSElem=="ticket" && lz_chat_data.SelectedGroup.Amount > 0 && lz_chat_get_wm_element('ticket') == null)
            {
                lz_chat_overlay_set_mode("chat",false);
                return;
            }
            ...
            var hideGroup = ( (lz_session.OVLWMSElem =="chat" && (lz_mode_change_details || lz_hide_group_chat)) || (lz_session.OVLWMSElem =="ticket" && lz_hide_group_ticket));
...
function lz_chat_show_feedback(){
    ...
    lz_mode_chat_feedback = !lz_mode_chat_feedback;
    lz_chat_update_ui_elements();
}
...
function lz_chat_kb_deactivate(_closeButton,_switchTo){
    ...
    if(_switchTo)
    {
        if(lz_chat_get_wm_element('knowledgebase') != null)
            lz_chat_overlay_set_mode('knowledgebase',true);
        else
...
function lz_chat_get_auto_mode(){
    ...
    if((OverlayChatWidgetV2.__ModeCombi || lz_chat_get_wm_element('chat') != null) && lz_chat_available && !lz_chat_declined && !OverlayChatWidgetV2.__TicketRequired)
        autoMode = 'chat';
...
function lz_chat_set_application_status(_hideWidget,_chat,_bot,_human,_title,_chatStatus,_declined){
    ...
    if(lz_overlay_chat != null)
    {
        ...
        lz_chat_declined = false;//_declined;
        ...
        lz_chat_available = _chat;

        lz_chat_validate_mode();
        lz_chat_update_ui_elements();
...
function lz_chat_validate_mode(){

    var autoMode = lz_chat_get_auto_mode();

    if(lz_overlay_wm != null && !OverlayChatWidgetV2.__ModeAPI)
        lz_overlay_wm.SetVisible(autoMode != 'NA' && !lz_overlay_chat.m_FixedMode && !(lz_overlay_chat.m_FullScreenMode && OverlayChatWidgetV2.__ModeSingle));
    ...
    if(lz_session.OVLWMSElem != autoMode)
    {
        ...
        {
            if(!lz_ticket_when_online && lz_chat_available)
            {
                lz_chat_overlay_set_mode(autoMode,false);
                return;
            }
...
function lz_chat_open(_playSound){
    ...
    {
        lz_chat_overlay_set_mode(lz_chat_get_auto_mode(),true);
    }
}
...
function lz_chat_hide(){
    ...
    lz_chat_overlay_set_mode(lz_chat_get_auto_mode());
}
...
function lz_chat_overlay_set_mode(type,click){

    var showBox;
    if(type == 'chat' && OverlayChatWidgetV2.__TicketRequired && click)
    {
        OverlayChatWidgetV2.__TicketRequired = false;
    }

    if(type == 'ticket' && lz_chat_handle_ticket_forward(click))
    {
    ...
    else
    {
        ...
        {
            type = '';
            if(click && document.getElementById("lz_chat_invite_id") != null && lz_chat_status == 0)
            {
        ...
        else
        {
            ...
            if(lz_is_mobile)
                showBox = (lz_session.OVLWMState=='1' && lz_session.OVLWMSElem != '' && type != 'wm');
            else
                showBox = (lz_session.OVLWMState=='1' && lz_session.OVLWMSElem != '');
        }

        if(click && !OverlayChatWidgetV2.__ModeSingle)
        {
            if(type == lz_session.OVLWMSElem && !lz_overlay_chat.m_FullScreenMode)
            {
                if(lz_chat_overlay_pointer != null && lz_session.OVLWMState!='0')
                    lz_chat_overlay_pointer.style.visibility = 'hidden';
            }
            if(type == 'knowledgebase' && document.getElementById('lz_chat_kb_match_info').style.display != 'none')
    ...
    if(click && lz_session.OVLWMSElem == 'chat')
        lz_chat_scoll_down();
...
function lz_chat_update_wm_ui(){
    ...
    for(var i=lz_ovlel.length-1;i>=0;i--)
    {
        ...
        else
        {
            if(lz_ovlel[i].type=='wm' && lz_chats_external)
            {
                ...
                cln.onclick = function(){lz_chat_overlay_set_mode(this.getAttribute('data-tid'),true);};
        }

        if(lz_ovlel[i].type == 'chat')
        {
            if(!lz_chat_available || lz_chat_declined)
                lz_add_class(cln,'disabled');
            else
                lz_remove_class(cln,'disabled');
        }
    ...
    if(lz_eye_catcher != null)
        lz_eye_catcher.SetVisible(lz_session.OVLWMState=='0');
}
