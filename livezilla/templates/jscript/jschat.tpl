...
{
    if(!lz_chat_data.CorrectMembers)
        return;

    for(var i=0;i<lz_chat_data.MembersPrevious.length;i++)
    {
        if(!lz_chat_is_room_member(lz_chat_data.Members,lz_chat_data.MembersPrevious[i].id))
        {
            var node = lz_chat_get_frame_object("").getElementById('rm_' + lz_chat_data.MembersPrevious[i].id);

            if(!(lz_chat_data.ShowDynGroupMemberActivities && lz_chat_data.DynamicGroup == ''))
    ...
    lz_chat_data.CorrectMembers = false;
    lz_chat_data.ShowDynGroupMemberActivities = lz_chat_data.DynamicGroup != '';
    lz_chat_data.MembersPrevious = lz_chat_data.Members;
    lz_chat_data.Members = [];
}

function lz_chat_update_room_member_list()
{
    for(var i=0;i<lz_chat_data.MembersPrevious.length;i++)
    {
        var user = lz_chat_data.MembersPrevious[i];
        if(!lz_chat_get_frame_object("").getElementById('rm_' + user.id))
        {
            var me = document.createElement('div');
            me.innerHTML = lz_global_base64_decode(user.name);
            me.id = 'rm_' + user.id;
            me.className = 'lz_chat_members_back_item ' + ((!user.isOp) ? 'lz_chat_members_back_external' : 'lz_chat_members_back_internal');
            me.onclick = function(xn){return function(){lz_chat_address_member(xn);}}(user.name);
            lz_chat_get_frame_object('lz_chat_members_box').appendChild(me);
        }

        if(lz_chat_get_frame_object("").getElementById('rm_' + user.id).innerHTML != lz_global_base64_decode(user.name))
            lz_chat_get_frame_object("").getElementById('rm_' + user.id).innerHTML = lz_global_base64_decode(user.name);

        lz_chat_get_frame_object("").getElementById('rm_' + user.id).style.fontStyle = (user.busyAway) ? 'italic' : 'normal';
    }

    if(lz_global_get_window_width(true) > 500 && (lz_chat_data.MembersPrevious.length >= 2 || lz_chat_data.DynamicGroup != ''))
    {
    ...
    {
        lz_chat_get_frame_object('lz_chat_members_box').style.display = 'none';
        lz_chat_get_frame_object('lz_chat_main').style.right = "0";
    }

    lz_chat_clear_room_members();
}

function lz_chat_is_room_member(_list,_id)
{
...
{
    lz_chat_data.CorrectMembers = true;

    _id = lz_global_base64_decode(_id);
    var exists = lz_chat_is_room_member(lz_chat_data.Members,_id);

    if(!exists)
    {
        if(lz_chat_data.ExternalUser != null && !lz_chat_is_room_member(lz_chat_data.Members,lz_chat_data.ExternalUser.Session.UserId+'~'+lz_chat_data.ExternalUser.Session.BrowserId))
            lz_chat_data.Members.push({id:lz_chat_data.ExternalUser.Session.UserId+'~'+lz_chat_data.ExternalUser.Session.BrowserId,name:lz_global_base64_encode(lz_global_htmlentities(lz_chat_data.ExternalUser.Username)),isOp:false,busyAway:false});

        if(!lz_chat_is_room_member(lz_chat_data.MembersPrevious,_id))
        {
            lz_chat_data.MembersPrevious.push({id:_id,name:_fullname,isOp:_isOperator,busyAway:_isBusyAway});
            if(!_inDynamicGroup || lz_chat_data.ShowDynGroupMemberActivities)
                lz_chat_add_system_text(1,_fullname);
        }
        lz_chat_data.Members.push({id:_id,name:_fullname,isOp:_isOperator,busyAway:_isBusyAway});
    }
}

function lz_chat_address_member(_name)
{
    lz_chat_get_frame_object('lz_chat_text').value = "@" + lz_global_base64_decode(_name) + " ";
    lz_chat_focus_textbox();
}
...
function lz_chat_set_host(_id,_fullname,_groupname,_language,_typing,_vcard,_dygroup,_feedback)
{
	...
	lz_chat_get_frame_object('lz_chat_operator_typing_info').style.display = (_id.length > 0 && _typing) ? "block" : "none";
...
{
    lz_chat_get_frame_object('lz_chat_operator_image').style.backgroundImage = "url("+lz_chat_data.TempImage.src+")";
}
...
function lz_chat_show_connected(_showConnecting,_showResume)
{
	if(!lz_chat_data.ConnectedMessageAppended)
	{
        ...
        if(_showConnecting)
            ...
            lz_chat_add_system_text(14,null);
	}
}
...
{
    var lml = " <a href=\"javascript:parent.lz_chat_goto_message(true,false);\">" + lz_chat_data.Language.LanguageLeaveMessageShort + "</a>. " + lz_chat_data.Language.ClientThankYou;
	var text = "";
	if(_type == 0)
		...
		text = lz_chat_data.Language.ClientIntLeft + lml;
	else if(_type == 4)
		text = lz_chat_data.Language.ClientIntDeclined + lml;
	else if(_type == 5)
		text = lz_chat_data.Language.ClientStillWaitingInt + lml;
	else if(_type == 6)
		text = lz_chat_data.Language.ClientIntIsConnected;
	else if(_type == 8)
		text = lz_chat_data.Language.ClientNoInternUsers + lml;
	else if(_type == 9)
        ...
        text = lz_chat_data.Language.ClientResume;
	else if(_type == 99)
		text = lz_global_base64_decode(_texta);
	else
		text = _texta;
	text = lz_global_replace_smilies(text,true);
    text = lz_global_base64_decode(lz_chat_data.Templates.MessageSystem).replace("<!--message-->",text);
...
{	
	...
	else if(_status == lz_chat_data.STATUS_INIT && lz_chat_data.TimerWaiting == null && <!--show_waiting_message--> > 0)
    {
    ...
    if(_status == lz_chat_data.STATUS_ACTIVE)
    {
        lz_chat_data.FeedbackURL = "./feedback.php?cid=" + lz_global_base64_url_encode(lz_chat_data.Id) + "<!--lang_param-->";

        if(lz_chat_data.FeedbackOnExit && lz_chat_get_frame_object('lz_chat_dialog_resource').src.indexOf('knowledgebase.php')===-1)
        ...
        lz_chat_get_frame_object('lz_chat_callback_feedback').className='lz_form_button lz_form_multi_button';
    }
}
...
function lz_chat_process_xml(_xml)
{
	try
	{
		if(_xml.length > 0 && _xml.indexOf("<livezilla_js>") != -1)
		{
            ...
            lz_chat_update_room_member_list();
		}
