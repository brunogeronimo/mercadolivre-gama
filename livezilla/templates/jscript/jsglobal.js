...
function lz_geo_resolver(){
	...
	function lz_resolver_connect_async()
	{
		...
		lz_resolver_request.src = lz_global_base64_decode(lz_geo_url) + "&gv=1021&method=" + OnEnd + "&spanm=" + OnSpan + "&oak=" + lz_oak + "&ip=" + lz_mip;
...
function lz_chat_get_input_value(_id){
    try{
        if(typeof lz_chat_data == 'undefined')
            return "";
        if(lz_chat_data.InputFieldValues==null)
            return "";
        return lz_chat_data.InputFieldValues[lz_array_indexOf(lz_chat_data.InputFieldIndices,_id)].Value;}
    catch(e)
    {
        console.log(e);
        return "";
...
function lz_group(_id,_amount,_description,_email,_document,_cihidden,_cimandatory,_tihidden,_timandatory,_cinf,_cmbinf,_tinf){
    this.Id = _id;
    this.Amount = _amount;
...
function lz_add_class(_el, _className) {
    ...
    else if (!lz_has_class(_el, _className)) _el.className += " " + _className
}
