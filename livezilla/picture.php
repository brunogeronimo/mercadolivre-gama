...
if(isset($_GET["intid"]) && Server::InitDataProvider())
{
    ...
    $id = Encoding::Base64UrlDecode($_GET["intid"]);
    if(isset(Server::$Operators[$id]))
    {
        $operator = Server::$Operators[$id];
		if(!isset($_GET["np"]))
...
if(function_exists("imagecreatetruecolor"))
{
    ...
    $parts = explode(" ",$text);
    $text = "";
    $text .= substr($parts[0],0,1);

    if(count($parts)>1)
        $text .= substr($parts[count($parts)-1],0,1);
    else if(strlen($parts[0])>1)
        $text .= substr($parts[0],1,1);
    ...
    $x_start = ceil(($max_width - $line_width[2] - $line_width[0]) / 2)-2;
    $y_start = ceil(($max_height /2) - (($line_width[5] - $line_width[0]) / 2));
